...
export async function getPendingInvoicesForParty(partyId: number): Promise<PendingInvoice[]> {
  const res  = await fetch(`/api/invoices?partyId=${partyId}&status=OPEN,PARTIAL&limit=200`);
  const body = await res.json().catch(() => ({}));
...
export async function getPartiesBasic(): Promise<PartyBasic[]> {
  try {
    ...
    const list: any[] = body.data ?? body.parties ?? (Array.isArray(body) ? body : []);
...
// FIX: correct backend URL is /api/party-ledger/party/:id/balance
// (was incorrectly /api/party-ledger/:id/balance — caused 404 silently)
export async function getPartyLedgerBalance(partyId: number): Promise<number> {
  try {
    const res  = await fetch(`/api/party-ledger/party/${partyId}/balance`);
    const body = await res.json().catch(() => ({}));
