...
function ChallanPaper({ challan, printRef }: { challan: ChallanItem; printRef: React.RefObject<HTMLDivElement> }) {
  ...
  const companyName = business.companyName || businessInfo.name        || "";
  const address     = business.address     || businessInfo.address     || "";
  const phone       = business.phone       || businessInfo.phone       || "";
  const mobile      = business.mobile      || businessInfo.mobile      || "";
  const panNo       = business.panNo       || businessInfo.panNo       || "";
  const gstin       = business.gstin       || businessInfo.gstin       || "";
  const bankName    = business.bankName    || businessInfo.bankName    || "";
  const accountNo   = business.accountNo  || businessInfo.accountNo   || "";
  const ifsc        = business.ifsc        || businessInfo.ifsc        || "";
  const branch      = business.branch      || businessInfo.branch      || "";
  const bankHolder  = business.accountName || business.bank || businessInfo.accountName || companyName;

  // Build a single address/contact line like the PDF
  const contactLine = [
    phone    ? `Phone: ${phone}` : "",
  ...
  const MetaRow = ({ label, value, minW = 120 }: { label: string; value: string | undefined | null; minW?: number }) => {
    ...
    return (
      <div style={{ display:"flex", gap:4, marginBottom:2 }}>
        <span style={{ fontWeight:700, minWidth:minW, flexShrink:0 }}>{label}</span>
        <span>: {value}</span>
      </div>
    );
  };
    ...
    >
      ...
      <div style={{ display:"grid", gridTemplateColumns:"120px 1fr 100px", alignItems:"center", padding:"10px 14px 8px", borderBottom:B, gap:8 }}>
        ...
        <div style={{ textAlign:"center" }}>
          ...
          {address && (
            <div style={{ fontSize:9.5, color:"#333", lineHeight:1.5 }}>{address}</div>
          )}
          {contactLine && (
            <div style={{ fontSize:9.5, color:"#333" }}>{contactLine}</div>
          )}
          {gstin && (
            <div style={{ fontSize:9.5, fontWeight:700, color:"#111", marginTop:1 }}>GSTIN NO : {gstin}</div>
          )}
        </div>
        {/* QR Code */}
      ...
      {/* ══ Row 2: Transport LEFT | Challan Meta RIGHT ══ */}
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", borderBottom:B }}>
        {/* LEFT — transport/vehicle info */}
        <div style={{ borderRight:B, padding:"5px 10px", fontSize:9.5 }}>
          ...
          {challan.financedBy && <MetaRow label="Financer's Name" value={challan.financedBy} />}
        </div>
        {/* RIGHT — challan identity */}
        <div style={{ padding:"5px 10px", fontSize:9.5 }}>
          <MetaRow label="Type of Invoice"  value="TAX"                         minW={100} />
            ...
            return (
              <tr key={item.id}>
                ...
                <td style={{ ...TD, textAlign:"center" as const }}>
                  {item.qty}<br/><span style={{ fontSize:8.5 }}>{item.unit || "PCS"}</span>
                </td>
    ...
    <div className="cvp-shell">
      ...
      <div className="cvp-body">
        ...
        <div className="cvp-sidebar">
          ...
          <div className="cvp-ph-row">
            <span>Date</span>
            <strong>{fmtDateGB(challan.date)}</strong>
          </div>
          <div className="cvp-ph-row">
            <span>Party</span>
            <strong>{challan.partyName}</strong>
          </div>
          ...
          {(challan.items||[]).map((item, i) => (
            <div key={i} className="cvp-ph-entry">
              ...
              <div className="cvp-ph-entry-date">
                Qty: {item.qty} {item.unit} × ₹{item.pricePerItem.toFixed(2)}
              </div>
            </div>
          ))}
      ...
      {/* Close dots backdrop */}
      {dotsOpen && (
        <div style={{ position:"fixed", inset:0, zIndex:499 }} onClick={() => setDotsOpen(false)}/>
      )}
    </div>
  );
}