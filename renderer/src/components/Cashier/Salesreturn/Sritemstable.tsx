      ...
      <div className="csr-col-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="csr-col-modal-body">
          <div className="csr-col-toggle-row">
            ...
            <button
              className={`csr-toggle${local.pricePerItem ? " csr-toggle--on" : ""}`}
          ...
          <div className="csr-col-toggle-row">
            ...
            <button
              className={`csr-toggle${local.quantity ? " csr-toggle--on" : ""}`}
          ...
          <div className="csr-col-notice">
            ...
            <span className="csr-col-notice-link">Items page (click here)</span>
          </div>
        </div>
  ...
  const updateItem = (rowId: string, field: string, value: any) => {
    onChange(
      items.map(item => {
        ...
        // sync taxRate when taxLabel changes
        if (field === "taxLabel") {
      ...
      <table className="csr-items-table">
        <thead>
          <tr>
            ...
            {/* + opens Show/Hide Columns modal */}
            <th className="csr-items-th csr-items-th--add">
              <button
                className="csr-add-col-btn"
        ...
        <tbody>
          {items.map((item, idx) => {
            ...
            return (
              <tr key={item.rowId} className="csr-items-row">
                {/* NO */}
                ...
                <td className="csr-items-td csr-items-td--name">
                  <input
                    className="csr-items-input csr-items-input--name"
                    value={item.name}
                    onChange={e => updateItem(item.rowId, "name", e.target.value)}
                  />
                  {item.description !== undefined && (
                    <input
                      className="csr-items-input csr-items-input--desc"
                ...
                <td className="csr-items-td">
                  <input
                    className="csr-items-input csr-items-input--sm"
                    value={item.hsn || ""}
                    onChange={e => updateItem(item.rowId, "hsn", e.target.value)}
                  />
                </td>

                {/* QTY — unit comes from linked invoice (read-only label, not dropdown) */}
                {columns.quantity && (
                  <td className="csr-items-td">
                    <div className="csr-items-qty-row">
                      <input
                        className="csr-items-input csr-items-input--num"
                        type="number"
                        min={0}
                        value={item.qty}
                        onChange={e => updateItem(item.rowId, "qty", Number(e.target.value))}
                      />
                      {/* Unit is display-only — it comes from the linked sales invoice */}
                      {item.unit && (
                        <span className="csr-items-unit-badge">{item.unit}</span>
                      )}
                    </div>
                  </td>
                )}
                  ...
                  <div className="csr-items-disc-col">
                    <div className="csr-items-disc-row">
                      ...
                      <input
                        ...
                        onChange={e => updateItem(item.rowId, "discountPct", Number(e.target.value))}
                      />
                    </div>
                    <div className="csr-items-disc-row">
                      <span className="csr-items-disc-symbol">₹</span>
                      <input
                        ...
                        onChange={e => updateItem(item.rowId, "discountAmt", Number(e.target.value))}
                      />
                    </div>
                  </div>
                </td>

                {/* Tax — dropdown + (₹ amount) sub-label below */}
                <td className="csr-items-td csr-items-td--tax">
                  <select
                    className="csr-items-tax-sel"
                    value={item.taxLabel}
                  ...
                  {/* Tax rupee amount shown below select — matches Image 1 */}
                  <div className="csr-items-tax-sub">(₹ {taxAmt.toFixed(0)})</div>
                </td>

                {/* Amount — ₹ symbol + value, matches Image 1 & 3 layout */}
                <td className="csr-items-td csr-items-td--amount">
                ...
                <td className="csr-items-td csr-items-td--del">
                  <button className="csr-items-del-btn" onClick={() => removeItem(item.rowId)}>
          ...
          {/* Add Item + Scan Barcode row — dashed box layout matching screenshot */}
          <tr className="csr-items-add-row">
            <td colSpan={colCount + 1} className="csr-items-add-cell">
              <button className="csr-add-item-btn" onClick={onAddItem}>
                + Add Item
              </button>
            </td>
            <td className="csr-items-scan-cell">
              {/* Scan Barcode opens the same Add Items modal */}
