...
import { Party, ShippingAddress, INDIAN_STATES } from "./Salesreturntypes";
import { fetchParties, createParty, PartyRecord } from "../../../api/partiesAndProductsApi";
import "./Createsalesreturn.css";

// ─── Map backend PartyRecord → frontend Party ─────────────────────────────────
function toParty(r: PartyRecord): Party {
  return {
    id:               r.id,
    name:             r.partyName || r.name,
    category:         r.partyType || "-",
    mobile:           r.mobileNumber || "-",
    type:             (r.partyType as Party["type"]) || "Customer",
    balance:          0,
    ...
    <div className="csr-overlay" onClick={onClose}>
      <div className="csr-modal csr-create-party-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="csr-modal-body">
          {error && <div className="csr-modal-error">{error}</div>}
          ...
          {!showAddress ? (
            <button className="csr-link-btn" onClick={() => setShowAddress(true)}>
              + Address (Optional)
            </button>
          ) : (
            <div className="csr-address-section">
              <div className="csr-address-section-hdr">
                ...
                <button className="csr-link-btn csr-link-btn--red" onClick={() => setShowAddress(false)}>
                  Remove
                </button>
              </div>
              <div className="csr-field">
                    ...
                    {stateOpen && (
                      <div className="csr-state-dropdown">
                        {filteredStates.map(s => (
                          <button key={s} className="csr-state-option"
                            onClick={() => { setState(s); setStateSearch(""); setStateOpen(false); }}>
                            {s}
                          </button>
                        ))}
                      </div>
          ...
          {!showGstin ? (
            <button className="csr-link-btn" onClick={() => setShowGstin(true)}>
              + GSTIN (Optional)
            </button>
          ) : (
            <div className="csr-address-section">
              <div className="csr-address-section-hdr">
                ...
                <button className="csr-link-btn csr-link-btn--red" onClick={() => setShowGstin(false)}>
                  Remove
                </button>
              </div>
              <div className="csr-field">
        ...
        <div className="csr-modal-footer">
          <button className="csr-btn-cancel" onClick={onClose} disabled={saving}>
            Cancel
          </button>
          <button className="csr-btn-primary" onClick={handleSave} disabled={saving}>
            {saving ? "Saving…" : "Save"}
          </button>
        </div>
      </div>
    ...
    <div className="csr-overlay csr-overlay--top" onClick={onClose}>
      <div className="csr-modal csr-shipping-edit-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="csr-modal-body">
          ...
          <div className="csr-field">
            <label className="csr-field-label--blue">Street Address</label>
            <textarea
              className="csr-textarea"
              value={street}
...
export function ChangeShippingAddressModal({
  party, selected, onClose, onSelect, onPartyUpdate,
  ...
  const handleSaveAddr = (addr: ShippingAddress) => {
    ...
    const updated = idx >= 0
      ? existing.map(a => a.id === addr.id ? addr : a)
      : [...existing, addr];
      ...
      <div className="csr-modal csr-shipping-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="csr-shipping-list">
          {allAddrs.map(addr => (
            <div
              key={addr.id}
              className={`csr-shipping-row${localSelected === addr.id ? " csr-shipping-row--sel" : ""}`}
            >
        ...
        <button className="csr-link-btn csr-link-btn--padded" onClick={() => setEditAddr("new")}>
...
export default function SRPartySelector({
  selectedParty, shipFrom, onSelectParty, onShipFromChange,
}: PartySelectorProps) {
  ...
  const loadParties = useCallback(async (search?: string) => {
    ...
    setPartyError(null);
    try {
      const records = await fetchParties(search);
      setAllParties(records.map(toParty));
    } catch (err: any) {
  ...
  // Load on first open
  useEffect(() => {
    if (searchOpen && allParties.length === 0 && !loadingParties) {
      loadParties();
    }
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => loadParties(searchVal || undefined), 300);
    return () => clearTimeout(t);
  }, [searchVal]); // eslint-disable-line react-hooks/exhaustive-deps

  // Client-side filter on top of fetched list (instant feel while typing)
  const filtered = allParties.filter(p => {
    if (!searchVal.trim()) return true;
    const q = searchVal.toLowerCase();
    return (
      p.name.toLowerCase().includes(q) ||
      (p.mobile && p.mobile !== "-" && p.mobile.includes(q))
    );
  });
          ...
          {!searchOpen ? (
            ...
            <div className="csr-party-search-wrap">
              ...
              <div className="csr-party-dropdown">
                ...
                {/* Error state */}
                {partyError && (
                  <div className="csr-party-error">
                    {partyError}
                    <button
                      className="csr-party-retry"
                      onClick={() => loadParties(searchVal || undefined)}
                    >
                      Retry
                    </button>
                  </div>
                ...
                {/* Loading state */}
                {loadingParties && !partyError && (
                  <div className="csr-party-loading">Loading parties…</div>
                )}

                {/* Empty state */}
                {!loadingParties && !partyError && filtered.length === 0 && (
                  <div className="csr-party-empty">
                    {searchVal ? `No party found for "${searchVal}"` : "No parties found"}
                  </div>
                )}

                {/* Results */}
                {!loadingParties && !partyError && filtered.map(p => (
                  <div key={p.id} className="csr-party-option" onClick={() => handleSelect(p)}>
                    <span className="csr-party-option-name">{p.name}</span>
                    <span className="csr-party-option-type">{p.category || p.type}</span>
                  </div>
                ))}

                {/* Create new */}
                <div
                  className="csr-party-create"
                  onClick={() => { setShowCreateParty(true); setSearchOpen(false); }}
                >
                  + Create Party
                </div>
              </div>
        ...
        {showCreateParty && (
          <CreatePartyModal
            ...
            onSave={p => {
              ...
              // Refresh list so newly created party appears next time
              loadParties();
            }}
          />
        )}
  ...
  // ── Party already selected — show info card ─────────────────────────────────
  return (
    <>
      <div className="csr-party-selected-grid">
        <div className="csr-party-col-box">
          <div className="csr-party-col-hdr">
            <span>Party Name</span>
            <button
              className="csr-change-btn"
              onClick={() => {
                setLocalParty(null);
                onSelectParty(null as any);
                setAllParties([]);   // force re-fetch on next open
              }}
            >
              Change Party
            </button>
          ...
          <div className="csr-party-col-body">
            ...
            {localParty.mobile && localParty.mobile !== "-" && (
              <div className="csr-party-detail">
                Phone Number: <strong>{localParty.mobile}</strong>
              </div>
            )}
            {localParty.gstin && (
              <div className="csr-party-detail">
                GSTIN: <strong>{localParty.gstin}</strong>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="csr-party-col-box">
          <div className="csr-party-col-hdr">
            <span>Ship From</span>
            <button className="csr-change-btn" onClick={() => setShowShipping(true)}>
          ...
          <div className="csr-party-col-body">
            ...
            {shipFrom?.street && (
              <div className="csr-party-detail">{shipFrom.street}</div>
            )}
            {!shipFrom && localParty.billingAddress && (
              <div className="csr-party-detail">{localParty.billingAddress}</div>
            )}
          </div>
        </div>
      </div>
