...
const IconKeyboard = () => (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="6" width="20" height="12" rx="2"/><path d="M6 10h.01M10 10h.01M14 10h.01M18 10h.01M8 14h8"/></svg>);
const IconChevronDown = () => (<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"/></svg>);
...
const IconBarcode = () => (<svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 5v14M7 5v14M11 5v14M15 5v14M19 5v14M21 5v3M21 16v3M3 5H6M3 16v3"/></svg>);
const IconColumns = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="3" x2="9" y2="21"/><line x1="15" y1="3" x2="15" y2="21"/></svg>);
...
const IconGoogle = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M12.545 10.239v3.821h5.445c-.712 2.315-2.647 3.972-5.445 3.972-3.332 0-6.033-2.701-6.033-6.032s2.701-6.032 6.033-6.032c1.498 0 2.866.549 3.921 1.453l2.814-2.814C17.503 2.988 15.139 2 12.545 2 7.021 2 2.543 6.477 2.543 12s4.478 10 10.002 10c8.396 0 10.249-7.85 9.426-11.748L12.545 10.239z"/></svg>);
const IconXCircle = () => (<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>);
const IconMinusCircle = () => (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="8" y1="12" x2="16" y2="12"/></svg>);
...
interface Props {
  ...
  editData?: any | null;  // can be FullInvoiceData or raw API detail object
  isEdit?: boolean;
...
// ─── Helper: extract FullInvoiceData from raw API detail payload ───────────────
function apiDetailToFullData(raw: any): FullInvoiceData {
  if (!raw) return {} as any;
  // If it has lineItems it's already a FullInvoiceData
  if (raw.lineItems) return raw;
  // Otherwise map from DB shape
 const party: Party | null = raw.party ? {
  id: raw.party.id,
  name: raw.party.name || raw.party.partyName,
  mobile: raw.party.mobileNumber || "",
  email: raw.party.email || "",
  gstin: raw.party.gstin || "",
  category: "",
  type: "Customer",
  balance: 0,
  billingAddress: raw.party.billingAddress || "",
} : null;
  const lineItems: InvoiceLineItem[] = (raw.items || []).map((item: any, idx: number) => ({
    id: item.id || idx,
    item: {
  ...
  const charges: AdditionalCharge[] = (raw.additionalCharges || []).map((c: any, idx: number) => ({
    id:      c.id || idx,
    label:   c.name,
  ...
  return {
    party,
    invoiceDate:      raw.proformaDate ? raw.proformaDate.split("T")[0] : todayStr(),
    paymentTerms:     raw.paymentTermsDays || "",
    expiryDate:       raw.validTill ? raw.validTill.split("T")[0] : "",
    showPaymentTerms: !!raw.validTill,
    eWayBill:         raw.ewayBillNo || "",
    challanNo:        raw.challanNo || "",
    financedBy:       raw.financedBy || "",
    salesman:         raw.salesman || "",
    emailId:          raw.emailId || "",
    warrantyPeriod:   raw.warrantyPeriod || "",
    lineItems,
    notes:            raw.notes || "",
    terms:            raw.termsConditions || "",
    showNotes:        !!raw.notes,
    showTerms:        !!raw.termsConditions,
    charges,
    showCharges:      charges.length > 0,
    discountType:     (raw.discountType as any) || "Discount After Tax",
    discountPct:      Number(raw.discountPct) || 0,
    discountAmt:      Number(raw.discountAmt) || 0,
    showDiscount:     (Number(raw.discountAmt) || 0) > 0,
    adjustType:       (raw.adjustType as any) || "+ Add",
    adjustAmt:        Number(raw.adjustAmt) || 0,
    autoRound:        raw.autoRoundOff || false,
    totalAmount:      Number(raw.totalAmount) || 0,
  };
}

const CreateProformaInvoice: React.FC<Props> = ({ nextNumber, settings, editData, isEdit, onSave, onSaveNew, onBack }) => {
  // Normalise editData to FullInvoiceData shape
  const ed: FullInvoiceData | null = editData ? apiDetailToFullData(editData) : null;
  ...
  const totalTax      = lineItems.reduce((s, li) => s + (li.amount * li.taxRate) / 100, 0);
  ...
  const handleSelectParty = (p: Party) => {
    ...
    if (p.billingAddress) {
      const addr: ShippingAddress = { id: 1, name: p.name, street: p.billingAddress, city: "", state: "", pincode: "" };
  ...
  const handlePaymentTermsChange = (val: string) => {
    ...
    if (!isNaN(n) && n >= 0) { setPaymentTerms(n); setExpiryDate(addDays(invoiceDate, n)); }
    else if (val === "") setPaymentTerms("");
  };
  ...
  const updateLineItem = (id: number, field: string, value: any) => {
    setLineItems(prev => prev.map(li => {
      ...
      if (field === "discountPct") {
        ...
        if (updated.discountPct > 0) updated.discountAmt = parseFloat((newBase * updated.discountPct / 100).toFixed(2));
      }
  ...
  const removeLineItem = (id: number) => setLineItems(prev => prev.filter(li => li.id !== id));

  // ── Charges ───────────────────────────────────────────────────────────────────
  const addCharge = () => setCharges(prev => [...prev, { id: Date.now(), label: "", amount: 0, taxType: "No Tax Applicable" }]);
  ...
  const buildInvoice = () => {
    ...
    const fullData: FullInvoiceData = {
      party, invoiceDate, paymentTerms, expiryDate, showPaymentTerms,
      eWayBill, challanNo, financedBy, salesman, emailId, warrantyPeriod,
      lineItems, notes, terms, showNotes, showTerms,
      charges, showCharges,
      discountType, discountPct, discountAmt, showDiscount,
      adjustType, adjustAmt, autoRound, totalAmount,
    };
    return {
      ...
      // extra fields the parent needs to build the API payload
      eWayBill, challanNo, financedBy, salesman, emailId, warrantyPeriod,
      charges, showCharges,
      discountType, discountPct, discountAmt, showDiscount,
      adjustType, adjustAmt, autoRound,
      paymentTerms, expiryDate, showPaymentTerms,
      shippingAddress: selectedShipping,
      customFieldValues,
      signatureUrl,
      showEmptySignatureBox: signatureMode === "empty",
      fullData,
    };
  };
  ...
  const handleSaveNew = async () => {
    ...
    try {
      ...
      setIsSaving(false); // reset BEFORE calling parent so re-mount gets fresh state
      await onSaveNew(invoice);
    } catch {
      setIsSaving(false);
    }
  };
    ...
    <div className="aa-cpi-root">
      ...
      <div className="aa-cpi-topbar">
        <div className="aa-cpi-topbar-left">
          <button className="aa-cpi-back-btn" onClick={onBack}><IconArrowLeft /></button>
          <h1 className="aa-cpi-page-title">
            {isEdit ? "Update Proforma Invoice" : "Create Proforma Invoice"}
          </h1>
        </div>
        <div className="aa-cpi-topbar-right">
          <button className="aa-cpi-keyboard-btn" title="Keyboard shortcuts"><IconKeyboard /></button>
          <button className="aa-cpi-settings-btn" onClick={() => { setTempSettings({ ...localSettings }); setShowSettingsModal(true); }}>
            <IconSettings /><span>Settings</span><span className="aa-cpi-settings-dot" />
          </button>
          {!isEdit && (
            <button className="aa-cpi-save-new-btn" onClick={handleSaveNew} disabled={isSaving}>
              Save &amp; New
            </button>
          )}
          <button
            className={isEdit ? "aa-cpi-update-btn" : "aa-cpi-save-btn"}
            onClick={handleSave}
            disabled={isSaving}
          >
            {isSaving ? "Saving…" : isEdit ? "Update Proforma Invoice" : "Save"}
          </button>
        </div>
      </div>
      ...
      <div className="aa-cpi-body">
        ...
        <div className="aa-cpi-right">
          ...
          {/* Payment Terms */}
          {!showPaymentTerms ? (
            <div className="aa-cpi-due-box" onClick={() => setShowPaymentTerms(true)}>+ Add Due Date</div>
          ) : (
            <div className="aa-cpi-payment-row">
              ...
              <div className="aa-cpi-terms-field">
                ...
                <div className="aa-cpi-terms-input-wrap">
                  <input type="number" className="aa-cpi-terms-input" value={paymentTerms} min={0} onChange={e => handlePaymentTermsChange(e.target.value)} />
              ...
              <div className="aa-cpi-expiry-field">
                ...
                <div className="aa-cpi-date-btn">
                  <input type="date" className="aa-cpi-date-input" value={expiryDate} onChange={e => setExpiryDate(e.target.value)} />
                </div>
              </div>
            </div>
          )}
          ...
          {/* Extra Fields */}
          {(() => {
            type FieldDef = { label: string; key: string; isCustom?: boolean; defaultValue?: string };
            ...
            const getVal = (f: FieldDef): string => {
              ...
              <div className="aa-cpi-extra-grid">
                {visibleFields.map(f => (
                  <div className="aa-cpi-extra-field" key={f.key}>
                    ...
                    <input className="aa-cpi-extra-input" value={getVal(f)} onChange={e => setVal(f, e.target.value)} />
                  </div>
                ))}
              </div>
            );
      ...
      <div className="aa-cpi-items-section">
                ...
                <tr key={li.id} className="aa-cpi-item-row">
                  ...
                  <td className="aa-cpi-td-item">
                    <div className="aa-cpi-item-name-cell">
                      ...
                      <input className="aa-cpi-desc-input" placeholder="Enter Description (optional)"
                        value={li.description} onChange={e => updateLineItem(li.id, "description", e.target.value)} />
                    </div>
                  </td>
                  ...
                  {colConfig.showQuantity && (
                    <td className="aa-cpi-td-qty">
                      <div className="aa-cpi-qty-cell">
                        <input className="aa-cpi-cell-input cpi-qty-num" type="number" min={1}
                          value={li.qty} onChange={e => updateLineItem(li.id, "qty", Number(e.target.value) || 1)} />
                  ...
                  {colConfig.showPricePerItem && (
                    <td className="aa-cpi-td-price">
                      <input className="aa-cpi-cell-input cpi-price-input" type="number" min={0}
                        value={li.pricePerItem} onChange={e => updateLineItem(li.id, "pricePerItem", Number(e.target.value) || 0)} />
                    </td>
                  )}
                  <td className="aa-cpi-td-disc">
                    <div className="aa-cpi-disc-cell">
                      <div className="aa-cpi-disc-row">
                        ...
                        <input className="aa-cpi-cell-input cpi-disc-input" type="number" min={0} max={100}
                          value={li.discountPct} onChange={e => updateLineItem(li.id, "discountPct", Number(e.target.value))} />
                      </div>
                      <div className="aa-cpi-disc-row">
                        <span className="aa-cpi-disc-label">₹</span>
                        <input className="aa-cpi-cell-input cpi-disc-input" type="number" min={0}
                          value={li.discountAmt} onChange={e => updateLineItem(li.id, "discountAmt", Number(e.target.value))} />
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="aa-cpi-td-amount">
                    <div className="aa-cpi-amount-cell">
                      ...
                      <input className="aa-cpi-cell-input cpi-amount-input" type="number"
                  ...
                  <td className="aa-cpi-td-del">
                    <button className="aa-cpi-remove-item" onClick={() => removeLineItem(li.id)}><IconTrash /></button>
                  </td>
                </tr>
              );
            })}
        ...
        <div className="aa-cpi-add-item-row">
          <div className="aa-cpi-add-item-btn-wrap">
            <button className="aa-cpi-add-item-btn" onClick={() => setShowAddItems(true)}>+ Add Item</button>
          </div>
          <div className="aa-cpi-scan-wrap">
            <IconBarcode /><span>Scan Barcode</span>
          </div>
        </div>
      ...
      <div className="aa-cpi-bottom">
        ...
        <div className="aa-cpi-bottom-left">
          {!showNotes ? (
            ...
            <div className="aa-cpi-expandable">
              ...
              <textarea className="aa-cpi-textarea" placeholder="Enter your notes" value={notes}
          ...
          {!showTerms ? (
            ...
            <div className="aa-cpi-expandable">
              ...
              <textarea className="aa-cpi-textarea" placeholder="Enter your terms and conditions" value={terms}
                onChange={e => setTerms(e.target.value)} rows={3} />
              <div className="aa-cpi-textarea-actions">
                <button className="aa-cpi-tx-btn"><IconGoogle /> <span className="aa-cpi-tx-label">G</span></button>
                <button className="aa-cpi-tx-btn cpi-wa-btn"><IconWhatsApp /></button>
              </div>
          ...
          {!showCharges && (() => {
            ...
            return selectedBank ? (
              <div className="aa-bm-selected-card">
                ...
                <div className="aa-bm-selected-detail-row"><span>Account Number: </span><strong>{selectedBank.accountNumber}</strong></div>
                {selectedBank.ifsc && <div className="aa-bm-selected-detail-row"><span>IFSC Code: </span><strong>{selectedBank.ifsc}</strong></div>}
                {selectedBank.bankName && <div className="aa-bm-selected-detail-row"><span>Bank &amp; Branch: </span><strong>{selectedBank.bankName}</strong></div>}
                {selectedBank.holderName && <div className="aa-bm-selected-detail-row"><span>Account Holder: </span><strong>{selectedBank.holderName}</strong></div>}
                <div className="aa-bm-selected-actions">
                  <button className="aa-cpi-add-link" onClick={() => setShowSelectBank(true)}>Change Bank Account</button>
                  <button className="aa-cpi-add-link" style={{ color: "#ef4444" }} onClick={() => setSelectedBankId(null)}>Remove</button>
                </div>
              </div>
            ) : (
              <button className="aa-cpi-add-link" onClick={() => setShowAddBank(true)}>+ Add Bank Account</button>
            );
          })()}
        </div>
        ...
        <div className="aa-cpi-bottom-right">
            ...
            <>
              {charges.map(charge => (
                <div key={charge.id} className="aa-cpi-charge-row">
                  <input className="aa-cpi-charge-label-input" placeholder="Enter charge (ex. Transport Charge)"
                    value={charge.label}
                    onChange={e => setCharges(prev => prev.map(c => c.id === charge.id ? { ...c, label: e.target.value } : c))} />
                  <span className="aa-cpi-charge-currency">₹</span>
                  <input className="aa-cpi-charge-amt-input" type="number" min={0}
                    value={charge.amount}
          ...
          {/* Tax summary */}
          {totalTax > 0 && (
            <div className="aa-cpi-taxable-row" style={{ color: "#64748b", fontSize: "12.5px" }}>
          ...
          {!showDiscount ? (
            ...
            <div className="aa-cpi-discount-row">
              <div className="aa-cpi-discount-type-wrap">
                <button className="aa-cpi-discount-type-btn" onClick={() => setShowDiscountTypeDropdown(s => !s)}>
              ...
              <div className="aa-cpi-disc-inputs-wrap">
                ...
                <input className="aa-cpi-disc-pct-input" type="number" min={0} max={100}
                  value={discountPct || ""} placeholder="0"
                  onChange={e => { const v = Number(e.target.value); setDiscountPct(v); setDiscountAmt(taxableAmount > 0 ? parseFloat((taxableAmount * v / 100).toFixed(2)) : 0); }} />
                <span className="aa-cpi-disc-sep">/</span>
                <span className="aa-cpi-disc-rs-label">₹</span>
                <input className="aa-cpi-disc-amt-input" type="number" min={0}
                  value={discountAmt || ""} placeholder="0"
                  onChange={e => { const v = Number(e.target.value); setDiscountAmt(v); setDiscountPct(taxableAmount > 0 ? parseFloat((v / taxableAmount * 100).toFixed(2)) : 0); }} />
          ...
          <div className="aa-cpi-round-row">
            ...
            <div className="aa-cpi-adjust-wrap">
                  ...
                  <div className="aa-cpi-adjust-menu">
                    {(["+ Add", "- Reduce"] as const).map(opt => (
                      <button key={opt} className={`aa-cpi-adj-opt${adjustType === opt ? " active" : ""}`}
                        onClick={() => { setAdjustType(opt); setShowAdjustDropdown(false); }}>
                        {opt}
                      </button>
                    ))}
                  </div>
              ...
              <input className="aa-cpi-adjust-input" type="number" min={0}
                value={adjustAmt || 0} onChange={e => setAdjustAmt(Number(e.target.value))} />
            </div>
          </div>
          ...
          <div className="aa-cpi-signature-section">
            ...
            <input ref={signatureFileRef} type="file" accept="image/*" style={{ display: "none" }}
              onChange={e => {
                const file = e.target.files?.[0];
                if (file) { setSignatureUrl(URL.createObjectURL(file)); setSignatureMode("image"); setShowSignatureModal(false); }
                e.target.value = "";
              }} />
          </div>
        </div>
        ...
        <div className="aa-cpi-modal-overlay" onClick={() => setShowSignatureModal(false)}>
          <div className="aa-cpi-sig-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="aa-cpi-sig-modal-body">
              <button className="aa-cpi-sig-option-card" onClick={() => { setShowSignatureModal(false); signatureFileRef.current?.click(); }}>
                <div className="aa-cpi-sig-option-icon">
                  <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#5b5fc7" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                </div>
                <span className="aa-cpi-sig-option-label">Upload Signature from Desktop</span>
              </button>
              <button className="aa-cpi-sig-option-card" onClick={() => { setSignatureMode("empty"); setSignatureUrl(""); setShowSignatureModal(false); }}>
                <div className="aa-cpi-sig-option-icon">
                  <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="#5b5fc7" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="6" width="20" height="12" rx="2"/></svg>
                </div>
      ...
      {/* ── Modals ── */}
      {showAddParty && <AddPartyModal onSelect={handleSelectParty} onClose={() => setShowAddParty(false)} />}
      {showAddItems && <AddItemsModal onAdd={handleAddItems} onClose={() => setShowAddItems(false)} />}
      {showColModal && (
        <ColumnVisibilityModal config={colConfig}
          onSave={cfg => { setColConfig(cfg); setShowColModal(false); }}
          onClose={() => setShowColModal(false)} />
      )}
      {showAddBank && (
        <AddBankAccountModal
          onSubmit={acc => { const newAcc = { ...acc, id: Date.now() }; setBankAccounts(prev => [...prev, newAcc]); setSelectedBankId(newAcc.id); setShowAddBank(false); }}
          onClose={() => setShowAddBank(false)} />
      )}
      {showSelectBank && (
        <SelectBankAccountModal accounts={bankAccounts} selectedId={selectedBankId}
          onSelect={id => setSelectedBankId(id)}
          onDone={() => setShowSelectBank(false)}
          onClose={() => setShowSelectBank(false)} />
      )}
      {showChangeShipping && (
        <ChangeShippingModal addresses={shippingAddresses} selectedId={selectedShippingId}
      ...
      {showAddShipping && (
        <AddShippingAddressModal initial={editingShipping} defaultName={party?.name}
...
const TaxSelectInline: React.FC<{ value: string; onChange: (v: string) => void }> = ({ value, onChange }) => (
  <select className="aa-cpi-tax-select" value={value} onChange={e => onChange(e.target.value)}>
    {TAX_LINE_OPTIONS.map(opt => <option key={opt} value={opt}>{opt}</option>)}
  </select>
);
  ...
  return (
    <div className="aa-cpi-tax-dropdown">
      <button className="aa-cpi-tax-btn" onClick={() => setOpen(s => !s)}>
        {value} <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
      </button>
      {open && (
