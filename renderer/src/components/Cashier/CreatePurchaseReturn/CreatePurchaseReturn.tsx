...
interface PRItem {
  ...
  qty:         number;
  price:       number;
  discount:    number;    // stored as ₹ amount
  discountPct: number;    // stored as % — NEW (bidirectional)
  tax:         number;
  taxLabel?:   string;    // e.g. "GST @ 18%" — NEW
}
...
const SEED_RETURNS: PurchaseReturn[] = [
  {
    id: 1, date: "2026-03-05", returnNumber: 1,
    partyName: "Cash Sale", partyId: 3, partyPhone: "9555780835",
    purchaseNo: "-", amount: 0, status: "credited",
    items: [
      { id: 2, name: "BILLING SOFTWARE WITH GST",    hsn: "", qty: 1, price: 0, discount: 0, discountPct: 0, tax: 0, taxLabel: "None" },
      { id: 3, name: "BILLING SOFTWARE WITHOUT GST", hsn: "", qty: 1, price: 0, discount: 0, discountPct: 0, tax: 0, taxLabel: "None" },
    ],
        ...
        <div className="pi-s-block">
          <div className="pi-s-top">
            ...
            <label className="toggle">
              <input
                type="checkbox"
                checked={prSettings.prefixEnabled}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setPrSettings((p) => ({ ...p, prefixEnabled: e.target.checked }))
                }
              />
            ...
            <>
              <div className="pi-s-fields">
                <div>
                  ...
                  <input
                    placeholder="Prefix"
                    value={prSettings.prefix}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setPrSettings((p) => ({ ...p, prefix: e.target.value }))
                    }
                  />
                </div>
                <div>
                  ...
                  <input
                    type="number"
                    value={prSettings.sequenceNumber}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setPrSettings((p) => ({
                        ...p,
              ...
              <p className="pi-s-note">
                Purchase Return Number: {prSettings.prefix}
                {prSettings.sequenceNumber}
              </p>
            </>
          )}
        </div>
        <div className="pi-s-block">
          <div className="pi-s-top">
            ...
            <label className="toggle">
              <input
                type="checkbox"
                checked={prSettings.showItemImage}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                  setPrSettings((p) => ({
                    ...p,
    ...
    <div className="pi-cal-overlay">
      <div className="pi-cal-header-row">
        <div className="pi-cal-section">
          ...
          <div className="pi-cal-section-value">{start ? fmtShort(start) : ""}</div>
        </div>
...
function BillViewPage({
  ...
  const discountAmt   = record.discountEnabled
    ? record.discountType === "%"
      ? (taxableAmount * record.discountVal) / 100
      : record.discountVal
    : 0;
  const roundOffAmt = record.roundOff
    ? record.roundOffDir === "+Add"
      ? record.roundOffVal
      : -record.roundOffVal
    : 0;
  ...
  const returnDateDisplay = `${dateObj
    .getDate()
    .toString()
    ...
    <div className="bv-page">
      ...
      <div className="bv-topbar">
        <div className="bv-title-wrap">
          <button className="bv-back-btn" onClick={onBack}>
            <IC.Back />
          </button>
          <span className="bv-page-title">
            Purchase Return #{record.returnNumber}
          </span>
          {record.status === "credited" && (
            <span className="pr-status-credited">Credited</span>
          )}
          {record.status === "pending" && (
            <span className="s-badge s-unpaid">Pending</span>
          )}
        </div>
        <div className="bv-topbar-actions">
          <button className="bv-action-btn">
            <IC.Download /> Download PDF
          </button>
          <button className="bv-action-btn">
            <IC.Print /> Print PDF
          </button>
          <button className="bv-action-btn bv-icon-btn">
            <IC.Refresh />
          </button>
          <button className="bv-action-btn">
            <IC.Share /> Share{" "}
          ...
          <div style={{ position: "relative" }}>
            <button
              ref={dotsRef}
              className={`bv-action-btn bv-dots-btn${showCtx ? " active" : ""}`}
            ...
            {showCtx && (
              <div ref={ctxRef} className="pi-ctx bv-ctx">
                ...
                <div
                  ...
                  onMouseEnter={(e) => hoverStyle(e, true)}
                  onMouseLeave={(e) => hoverStyle(e, false)}
                  onClick={() => doCtxAction("duplicate")}
                >
                  <IC.Copy /> Duplicate
                </div>
                <div className="pi-dd-divider" />
                <div
                  className="pi-dd-item danger"
                  onMouseEnter={(e) => hoverStyle(e, true, true)}
                  onMouseLeave={(e) => hoverStyle(e, false, true)}
                  onClick={() => doCtxAction("delete")}
                >
                  <IC.Trash /> Delete
                </div>
              </div>
        ...
        <div className="bv-paper">
          ...
          <table className="bv-items-table">
            ...
            <tbody>
              ...
              {/* padding rows */}
              {record.items.length < 6 &&
                Array.from({
                  length: Math.max(0, 4 - record.items.length),
                }).map((_, i) => (
                  <tr key={`pad-${i}`}>
                    <td colSpan={6} style={{ height: 34 }} />
                  </tr>
                ))}
            </tbody>
          </table>
          ...
          <div className="bv-totals-row">
            ...
            <div className="bv-totals-qty">
              {record.items.reduce((s, i) => s + i.qty, 0)}
            </div>
            <div className="bv-totals-tax">
              {totalTax > 0
                ? `₹ ${totalTax.toLocaleString("en-IN")}`
                : "₹ 0"}
            </div>
            <div className="bv-totals-amt">
              ₹ {totalAmt.toLocaleString("en-IN")}
            </div>
          </div>
          <div className="bv-received-row">
            <div className="bv-received-label">RECEIVED AMOUNT</div>
            <div className="bv-received-amt">
              ₹ {(record.amtReceived || 0).toLocaleString("en-IN")}
            </div>
          </div>
          ...
          <table className="bv-tax-table">
            <thead>
              <tr>
                ...
                <th
                  colSpan={2}
                  style={{ textAlign: "center", borderLeft: "1px solid #eaecf0" }}
                >
                  CGST
                </th>
                <th
                ...
                <th
                  style={{ textAlign: "right", borderLeft: "1px solid #eaecf0" }}
                >
                  Total Tax Amount
                </th>
              </tr>
              <tr className="bv-tax-sub-head">
                <th />
                <th />
                <th style={{ borderLeft: "1px solid #eaecf0" }}>Rate</th>
                <th>Amount</th>
                <th style={{ borderLeft: "1px solid #eaecf0" }}>Rate</th>
                <th>Amount</th>
                <th />
            ...
            <tbody>
              <tr>
                ...
                <td style={{ textAlign: "center", borderLeft: "1px solid #eaecf0" }}>
                  0%
                </td>
                <td style={{ textAlign: "center" }}>
                  {totalTax > 0 ? (totalTax / 2).toLocaleString("en-IN") : 0}
                </td>
                <td style={{ textAlign: "center", borderLeft: "1px solid #eaecf0" }}>
                  0%
                </td>
          ...
          <div className="bv-footer-row">
            ...
            <div className="bv-sig-block">
              <div className="bv-sig-label">
                Authorized signatory for{" "}
                <strong>scratchweb.solutions</strong>
              </div>
              {/* Signature — saved during Create/Edit, displayed here */}
              <div
                className="bv-sig-box"
...
function PurchaseReturnForm({
  ...
  const initParty = initialData
    ? allParties.find((p) => p.id === initialData.partyId) || null
    : null;
  ...
  // ── Signature upload state (lives here — saved into record on submit) ────
  ...
  const taxableAmount = subtotal + chargesTotal;
  const discountAmt   = showDiscount
    ? discountType === "%"
  ...
  const filtInvoices = PURCHASE_INVOICES.filter((i) =>
    i.toLowerCase().includes(invSearch.toLowerCase()),
  );
  ...
  const setPendingQty = (id: number, v: number): void =>
  ...
  const updItem = (id: number, f: keyof PRItem, v: string): void =>
    setItems((p) =>
      p.map((i) =>
        i.id === id ? { ...i, [f]: isNaN(Number(v)) ? v : Number(v) } : i,
      ),
    );

  /* ── Bidirectional discount handlers ── */
  const updateDiscountPct = (id: number, pct: number, item: PRItem): void => {
    const rs = parseFloat(((item.qty * item.price * pct) / 100).toFixed(2));
    setItems((p) =>
      p.map((i) => (i.id === id ? { ...i, discountPct: pct, discount: rs } : i)),
    );
  };

  const updateDiscountRs = (id: number, rs: number, item: PRItem): void => {
    const base = item.qty * item.price;
    const pct  = base > 0 ? parseFloat(((rs / base) * 100).toFixed(2)) : 0;
    setItems((p) =>
      p.map((i) => (i.id === id ? { ...i, discount: rs, discountPct: pct } : i)),
    );
  };

  /* ── Tax label + rate handler ── */
  const updateItemTax = (id: number, label: string, rate: number): void => {
    setItems((p) =>
      p.map((i) => (i.id === id ? { ...i, tax: rate, taxLabel: label } : i)),
    );
  };

  const addCharge = (): void =>
    setCharges((p) => [
      ...p,
  ...
  const removeCharge = (id: number): void =>
  ...
  const buildReturn = (
    ...
    purchaseNo:     linkedInvoice || "-",
    amount:         totalAmt,
    status:
      markPaid || amtReceived >= totalAmt
        ? "credited"
        : totalAmt > 0
          ? "pending"
          : "",
    ...
    <div className="cpi-page">
      <div className="cpi-topbar">
        ...
        <div className="cpi-topbar-right">
          {isEdit && (
            <button
              className="btn-icon btn-grid-icon"
              onClick={() => showT("Grid view")}
            >
              <IC.Monitor />
            </button>
          )}
          ...
          <button className="btn-save-top" onClick={handleSave}>
            {isEdit ? "Update Purchase Return" : "Save"}
          </button>
        </div>
      </div>
      ...
      <div className="cpi-body">
              ...
              <div className="bill-ship-split">
                ...
                <div className="bill-ship-pane">
                  ...
                  {shipSameAsBill ? (
                    <>
                      <div className="party-info-name">{selectedParty.name}</div>
                      <div className="party-info-line ship-same-label">
                        Same as Bill From
                      </div>
                    </>
                  ) : (
          ...
          <div className="cpi-invoice-fields">
            ...
            {!isEdit && (
              <div className="pr-link-section" ref={invRef}>
                <div className="pr-link-label">
                  Link to Purchase Invoice
                </div>
                <div
                  className="pr-link-search"
                  onClick={() => setShowInvDrop(true)}
                >
                  <span className="pr-link-icon">
                    <IC.Search />
                  </span>
                  <input
            ...
            <div className="extra-fields-grid">
              <div className="extra-field">
                <label>E-Way Bill No. <IC.Info /></label>
                <input />
              </div>
              <div className="extra-field">
                  ...
                  <tr key={item.id}>
                    ...
                    <td>
                      <input
                        ...
                        value={item.hsn}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          updItem(item.id, "hsn", e.target.value)
                        }
                      />
                    </td>
                    <td>
                      <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
                        <input
                          ...
                          value={item.qty}
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                            updItem(item.id, "qty", e.target.value)
                          }
                        />
                    ...
                    <td>
                      <input
                        ...
                        placeholder="0"
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                          updItem(item.id, "price", e.target.value)
                        }
                      />
                    </td>

                    {/* ── DISCOUNT CELL — bidirectional % ↔ ₹ ── */}
                    <td>
                      <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                        {/* % row */}
                        <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
                          <span style={{ fontSize: 11, color: "#667085", minWidth: 10 }}>%</span>
                          <input
                            className="qty-input"
                            style={{ width: 68 }}
                            type="number"
                            ...
                            placeholder="0"
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                              updateDiscountPct(item.id, Number(e.target.value) || 0, item)
                            }
                          />
                        </div>
                        {/* ₹ row */}
                        <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
                          ...
                          <input
                            ...
                            placeholder="0"
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                              updateDiscountRs(item.id, Number(e.target.value) || 0, item)
                            }
                          />
                        </div>
                      </div>
                    ...
                    {/* ── TAX CELL — GST dropdown ── */}
                    <td style={{ minWidth: 170 }}>
                      <TaxDropdown
                        value={item.taxLabel ?? "None"}
                        onChange={(label, rate) =>
                          updateItemTax(item.id, label, rate)
                        }
                      />
                      {item.tax > 0 && (
                        <div style={{ fontSize: 10, color: "#667085", marginTop: 3 }}>
                          ₹{" "}
                          {(
        ...
        <div className="cpi-bottom-section">
          ...
          <div className="summary-panel">
            {charges.map((c) => (
              <div key={c.id} className="charge-row">
                <input
                  ...
                  value={c.label}
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                    updCharge(c.id, "label", e.target.value)
                  }
                />
                <div className="charge-right">
                  ...
                  <select
                    ...
                    onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                      updCharge(c.id, "taxRate", e.target.value)
                    }
                  >
              ...
              <div className="discount-row">
                ...
                <div className="discount-inputs">
                  ...
                  <input
                    ...
                    value={discountVal}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setDiscountVal(Number(e.target.value))
                    }
                  />
              ...
              {roundOff && (
                <div className="round-off-controls">
                  ...
                  <input
                    ...
                    value={roundOffVal}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setRoundOffVal(Number(e.target.value))
                    }
                  />
                </div>
              )}
              ...
              <label className="mark-paid-label">
                ...
                <input
                  ...
                  onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                    ...
                    if (e.target.checked) setAmtReceived(totalAmt);
                    else setAmtReceived(0);
                  }}
                />
              </label>
            </div>
              ...
              <div className="amount-paid-inputs">
                <div className="rupee-input-wrap">
                  ...
                  <input
                    ...
                    value={amtReceived}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                      setAmtReceived(Number(e.target.value))
                    }
                  />
                </div>
                <select
            ...
            {/* ── Signature Upload — hidden input + clickable sig-box ── */}
            <input
              ref={sigInputRef}
              type="file"
              accept="image/*"
                ...
                <div className="optional-section">
                  ...
                  <label className="ship-checkbox">
                    <input
                      ...
                      checked={cpSameShip}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        setCpSameShip(e.target.checked)
                      }
                    />
      ...
      {showAddItems && (
        <div
          className="modal-overlay"
          onClick={() => { setShowAddItems(false); setItemSearch(""); }}
        >
          <div
            className="aim-modal"
            onClick={(e: React.MouseEvent) => e.stopPropagation()}
                ...
                <tbody>
                  {filtCatalog.map((c) => {
                    ...
                    return (
                      <tr
                        key={c.id}
                        className={isAdded ? "aim-row-added" : ""}
                      >
...
function PurchaseReturnListPage({
    ...
    return returns.filter((r) => {
      ...
      if (query.trim()) {
        ...
        return (
          ...
          r.returnNumber.toString().includes(q)
        );
      }
      return true;
    });
  ...
  const doCtx = (action: string): void => {
    ...
    if (action === "delete") {
      setReturns((p) => p.filter((i) => i.id !== r.id));
      showT(`Return #${r.returnNumber} deleted`);
    }
  };
  ...
  return (
    <div className="pi-page pr-list-page">
      ...
      <div className="pr-toolbar">
        {/* ── Search input ── */}
        <div className="pi-search-group" style={{ flex: 1, maxWidth: 420 }}>
        ...
        <div className="pi-date-wrap" ref={dateRef}>
          <button
            className="pi-date-btn pr-date-btn"
            onClick={() => {
              setShowCalendar(false);
              setShowDateList((v) => !v);
            }}
          >
            <IC.Calendar /> {dateBtnLabel()}{" "}
        ...
        <button className="btn-create-pr" onClick={onCreateNew}>
            ...
            {displayed.length === 0 ? (
                    ...
                    {query.trim() ? (
                      <>
                        ...
                        <div style={{ fontSize: 12, color: "#98a2b3" }}>
                          Try searching by party name or return number
                        </div>
                        <button
                          onClick={() => setQuery("")}
                          style={{
              ...
              displayed.map((r) => (
                ...
                >
                  ...
                  <td>{highlight(r.returnNumber.toString(), query)}</td>
                  <td>{highlight(r.partyName, query)}</td>
                  <td>{r.purchaseNo}</td>
                  <td className="td-amt">
  ...
  if (page === "edit" && activeRecord)
    return (
      <PurchaseReturnForm
        ...
        {...commonProps}
        mode="edit"
        seqNo={activeRecord.returnNumber}
  ...
  if (page === "duplicate" && activeRecord)
    return (
      <PurchaseReturnForm
        ...
        {...commonProps}
        mode="duplicate"
        seqNo={prSettings.sequenceNumber}
        initialData={activeRecord}
        onSaved={handleSaved}
