...
function AdvancedGSTLayout({ invoice, business, template, printRef, themeColor, bgImageUrl, bgOpacity }: Omit<InvoicePaperProps,"themeLayout">) {
  ...
  const B = "1px solid #bbb";
  const NCOLS = 8;
  const TH: React.CSSProperties = { border: B, padding: "5px 8px", fontSize: 11, fontWeight: 700, background: themeColor, color: "#fff", textAlign: "center" as const, whiteSpace: "nowrap" as const };
  const TD: React.CSSProperties = { border: B, padding: "5px 8px", fontSize: 11, color: "#1a1a1a", verticalAlign: "top" as const };
  const metaLabel: React.CSSProperties = { fontSize: 8.5, color: "#666", fontWeight: 700, textTransform: "uppercase" as const, marginBottom: 1 };
  const metaValue: React.CSSProperties = { fontSize: 10.5, fontWeight: 700 };
  const metaValueNormal: React.CSSProperties = { fontSize: 10.5 };
  return (
    <div ref={printRef} className="ivm-invoice-paper" style={{ fontFamily: "Arial, sans-serif", fontSize: "11px", color: "#1a1a1a", border: "1px solid #bbb", background: "#fff", padding: 0, overflow: "hidden", width: "100%", maxWidth: "100%", margin: 0, position: "relative" }}>
      <BgOverlay url={bgImageUrl} opacity={bgOpacity} />
      <div style={{ position: "relative", zIndex: 1 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "5px 12px", borderBottom: B }}>
          <span style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.03em" }}>TAX INVOICE</span>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", borderBottom: B }}>
          <div style={{ padding: "8px 12px", borderRight: B, display: "flex", alignItems: "flex-start", gap: 10 }}>
            {showLogo && logoUrl && (<img src={logoUrl} alt="Logo" style={{ height: 52, width: 52, objectFit: "contain", borderRadius: 4, flexShrink: 0, border: "1px solid #eee" }} />)}
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 800, color: themeColor, marginBottom: 2 }}>{companyName}</div>
              <div style={{ fontSize: 10.5, color: "#333", marginBottom: 1 }}>{address}</div>
              {phone && <div style={{ fontSize: 10.5, color: "#333", marginBottom: 1 }}>Phone: {phone}</div>}
        ...
        <div style={{ display: "grid", gridTemplateColumns: invoice.shipTo ? "1fr 1fr" : "1fr", borderBottom: B }}>
          <div style={{ padding: "7px 12px", borderRight: invoice.shipTo ? B : "none" }}>
            <div style={{ fontSize: 9, fontWeight: 700, color: "#444", textTransform: "uppercase" as const, marginBottom: 4 }}>BILL TO</div>
            {invoice.party ? (<><div style={{ fontSize: 11.5, fontWeight: 700, color: "#111", marginBottom: 2 }}>{invoice.party.name}</div>{invoice.party.billingAddress && <div style={{ fontSize: 10.5, color: "#333", lineHeight: 1.5, marginBottom: 1 }}>Address: {invoice.party.billingAddress}</div>}{invoice.party.gstin && <div style={{ fontSize: 10.5, color: "#333", marginBottom: 1 }}>GSTIN: {invoice.party.gstin}</div>}{invoice.party.mobile && <div style={{ fontSize: 10.5, color: "#333" }}>Mobile: {invoice.party.mobile}</div>}</>) : <div style={{ fontSize: 10.5, color: "#aaa" }}>–</div>}
          </div>
          {invoice.shipTo && (<div style={{ padding: "7px 12px" }}><div style={{ fontSize: 9, fontWeight: 700, color: "#444", textTransform: "uppercase" as const, marginBottom: 4 }}>SHIP TO</div><div style={{ fontSize: 11.5, fontWeight: 700, color: "#111", marginBottom: 2 }}>{invoice.shipTo.name}</div>{invoice.shipTo.billingAddress && <div style={{ fontSize: 10.5, color: "#333", lineHeight: 1.5, marginBottom: 1 }}>Address: {invoice.shipTo.billingAddress}</div>}{invoice.shipTo.mobile && <div style={{ fontSize: 10.5, color: "#333" }}>Mobile: {invoice.shipTo.mobile}</div>}</div>)}
        </div>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <colgroup><col style={{ width: "5%" }}/><col style={{ width: "27%" }}/><col style={{ width: "9%" }}/><col style={{ width: "9%" }}/><col style={{ width: "11%" }}/><col style={{ width: "13%" }}/><col style={{ width: "9%" }}/><col style={{ width: "17%" }}/></colgroup>
          <thead><tr>{["S.NO.","ITEMS","HSN","QTY.","RATE","DISCOUNT","TAX","AMOUNT"].map((h,i) => (<th key={h} style={{ ...TH, textAlign: (i===1 ? "left" : "center") as any }}>{h}</th>))}</tr></thead>
          <tbody>
            {invoice.billItems.map((item, idx) => {
              const raw = item as any; const unitLabel = item.unit || "PCS"; const pct = Number(raw.discountPct ?? raw.discount_pct ?? 0) || 0; const amt = Number(raw.discountAmt ?? raw.discount_amt ?? 0) || 0; const base = Number(item.qty) * Number(item.price); const taxRate = Number(raw.taxRate ?? raw.tax_rate ?? 0) || 0; const taxLabel = taxRate > 0 ? (raw.taxLabel || `GST ${taxRate}%`) : "-";
              return (<tr key={idx}><td style={{ ...TD, textAlign: "center" as const }}>{idx + 1}</td><td style={{ ...TD }}><div style={{ fontWeight: 600 }}>{item.name || "Item"}</div>{item.description && <div style={{ fontSize: 9.5, color: "#555", marginTop: 1 }}>{item.description}</div>}</td><td style={{ ...TD, textAlign: "center" as const }}>{item.hsn || "-"}</td><td style={{ ...TD, textAlign: "center" as const }}>{item.qty} {unitLabel}</td><td style={{ ...TD, textAlign: "right" as const }}>{Number(item.price).toLocaleString("en-IN", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td><td style={{ ...TD, textAlign: "center" as const }}>{pct > 0 ? <span>{pct}%<br/><span style={{ fontSize:10,color:"#555" }}>({fmtN(Math.round(base*(pct/100)*100)/100)})</span></span> : amt > 0 ? <span>{fmtN(amt)}</span> : <span>-</span>}</td><td style={{ ...TD, textAlign: "center" as const }}>{taxRate > 0 ? taxLabel : "-"}</td><td style={{ ...TD, textAlign: "right" as const, fontWeight: 600 }}>{fmtN(Number(item.amount))}</td></tr>);
            })}
            {Array.from({ length: fillerCount }).map((_,i) => (<tr key={`filler${i}`} style={{ height: 22 }}>{Array.from({ length: NCOLS }).map((__,j) => <td key={j} style={{ ...TD, borderTop:"none",borderBottom:"none" }}/>)}</tr>))}
            <tr style={{ height:0 }}>{Array.from({ length: NCOLS }).map((_,j) => <td key={j} style={{ ...TD,padding:0,height:0,borderTop:B,borderBottom:"none" }}/>)}</tr>
            {taxRows.map((row,i) => [
              <tr key={`cgst-${i}`}><td colSpan={5} style={{ border:"none",borderRight:B,padding:"4px 8px",fontSize:10.5,fontStyle:"italic" as const,color:"#333",textAlign:"right" as const }}>CGST @{row.rate/2}%</td><td style={{ ...TD,textAlign:"center" as const,color:"#555",fontSize:10.5 }}>-</td><td style={{ ...TD,textAlign:"center" as const,color:"#555",fontSize:10.5 }}>-</td><td style={{ ...TD,textAlign:"right" as const,fontWeight:600 }}>₹ {fmtN(row.cgst)}</td></tr>,
              <tr key={`sgst-${i}`}><td colSpan={5} style={{ border:"none",borderRight:B,padding:"4px 8px",fontSize:10.5,fontStyle:"italic" as const,color:"#333",textAlign:"right" as const }}>SGST @{row.rate/2}%</td><td style={{ ...TD,textAlign:"center" as const,color:"#555",fontSize:10.5 }}>-</td><td style={{ ...TD,textAlign:"center" as const,color:"#555",fontSize:10.5 }}>-</td><td style={{ ...TD,textAlign:"right" as const,fontWeight:600 }}>₹ {fmtN(row.sgst)}</td></tr>,
            ])}
            {invoice.additionalCharges.map((c,i) => (<tr key={`charge-${i}`}><td colSpan={NCOLS-1} style={{ ...TD,textAlign:"right" as const,fontSize:10.5 }}>{c.label||"Additional Charge"}</td><td style={{ ...TD,textAlign:"right" as const,fontWeight:600 }}>₹ {fmtN(Number(c.amount))}</td></tr>))}
            {discountAmt > 0 && (<tr><td colSpan={NCOLS-1} style={{ ...TD,textAlign:"right" as const,fontSize:10.5 }}>Discount</td><td style={{ ...TD,textAlign:"right" as const,fontWeight:600,color:"#16a34a" }}>- ₹ {fmtN(discountAmt)}</td></tr>)}
            {roundOff !== 0 && (<tr><td colSpan={NCOLS-1} style={{ ...TD,textAlign:"right" as const,fontSize:10.5 }}>Round Off</td><td style={{ ...TD,textAlign:"right" as const,fontWeight:600 }}>{roundOff>0?"+":""}₹ {fmtN(Math.abs(roundOff))}</td></tr>)}
            <tr><td colSpan={6} style={{ ...TD,fontWeight:700,textAlign:"center" as const,fontSize:11,letterSpacing:"0.06em",background:"#f0f0f0",textTransform:"uppercase" as const }}>TOTAL</td><td style={{ ...TD,textAlign:"center" as const,fontWeight:600,background:"#f0f0f0",color:"#555" }}>-</td><td style={{ ...TD,textAlign:"right" as const,fontWeight:700,fontSize:11,background:"#f0f0f0" }}>₹ {fmtN(grandTotal)}</td></tr>
            <tr><td colSpan={NCOLS-1} style={{ ...TD,fontWeight:700,textAlign:"right" as const,fontSize:11,textTransform:"uppercase" as const }}>RECEIVED AMOUNT</td><td style={{ ...TD,textAlign:"right" as const,fontWeight:700,fontSize:11 }}>₹ {fmtN(alreadyReceived)}</td></tr>
            <tr><td colSpan={NCOLS-1} style={{ ...TD,fontWeight:700,textAlign:"right" as const,fontSize:11,textTransform:"uppercase" as const }}>BALANCE AMOUNT</td><td style={{ ...TD,textAlign:"right" as const,fontWeight:700,fontSize:11,color:safeN(grandTotal-alreadyReceived)>0?"#dc2626":"#16a34a" }}>₹ {fmtN(safeN(grandTotal-alreadyReceived))}</td></tr>
          </tbody>
        </table>
        {hsnRows.length > 0 && (
          <table style={{ width:"100%",borderCollapse:"collapse" }}>
            <thead>
              ...
              <tr>{["Rate","Amount","Rate","Amount"].map(h => <th key={h} style={{ ...TD,fontWeight:700,background:"#f0f0f0",fontSize:10,textAlign:"center" as const }}>{h}</th>)}</tr>
            </thead>
            <tbody>
              {hsnRows.map((row,i) => (<tr key={i}><td style={{ ...TD }}>{row.hsn}</td><td style={{ ...TD,textAlign:"right" as const }}>{fmtN(row.taxable)}</td><td style={{ ...TD,textAlign:"center" as const }}>{row.rate/2}%</td><td style={{ ...TD,textAlign:"right" as const }}>{fmtN(row.cgst)}</td><td style={{ ...TD,textAlign:"center" as const }}>{row.rate/2}%</td><td style={{ ...TD,textAlign:"right" as const }}>{fmtN(row.sgst)}</td><td style={{ ...TD,textAlign:"right" as const,fontWeight:600 }}>₹ {fmtN(row.cgst+row.sgst)}</td></tr>))}
              <tr style={{ background:"#f0f0f0" }}><td style={{ ...TD,fontWeight:700 }}>Total</td><td style={{ ...TD,textAlign:"right" as const,fontWeight:600 }}>{fmtN(hsnRows.reduce((s,r)=>s+r.taxable,0))}</td><td style={{ ...TD }}/><td style={{ ...TD,textAlign:"right" as const,fontWeight:600 }}>{fmtN(hsnRows.reduce((s,r)=>s+r.cgst,0))}</td><td style={{ ...TD }}/><td style={{ ...TD,textAlign:"right" as const,fontWeight:600 }}>{fmtN(hsnRows.reduce((s,r)=>s+r.sgst,0))}</td><td style={{ ...TD,textAlign:"right" as const,fontWeight:700 }}>₹ {fmtN(hsnRows.reduce((s,r)=>s+r.cgst+r.sgst,0))}</td></tr>
            </tbody>
          </table>
        )}
        <div style={{ padding:"6px 12px",borderTop:B,borderBottom:B,fontSize:11 }}><strong>Total Amount (in words): </strong>{numToWords(grandTotal)}</div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",borderBottom:B }}>
          <div style={{ borderRight:B,padding:"8px 12px",minHeight:64 }}><div style={{ fontSize:10.5,fontWeight:700,color:"#222",marginBottom:4 }}>Notes</div><div style={{ fontSize:10.5,color:"#333",lineHeight:1.6,whiteSpace:"pre-line" as const }}>{invoice.notes||""}</div></div>
          <div style={{ padding:"8px 12px",minHeight:64 }}><div style={{ fontSize:10.5,fontWeight:700,color:"#222",marginBottom:4 }}>Bank Details</div><div style={{ fontSize:10.5,color:"#333",lineHeight:1.9 }}>{bank && <div><span style={{ display:"inline-block",minWidth:90 }}>Name:</span>{bank}</div>}{bankName && <div><span style={{ display:"inline-block",minWidth:90 }}>Bank:</span>{bankName}</div>}{accountNo && <div><span style={{ display:"inline-block",minWidth:90 }}>A/C No.:</span>{accountNo}</div>}{branch && <div><span style={{ display:"inline-block",minWidth:90 }}>Branch:</span>{branch}</div>}{ifsc && <div><span style={{ display:"inline-block",minWidth:90 }}>IFSC:</span>{ifsc}</div>}</div></div>
        </div>
        <div style={{ padding:"8px 12px",borderBottom:B }}><div style={{ fontSize:10.5,fontWeight:700,color:"#222",marginBottom:4 }}>Terms and Conditions</div><div style={{ fontSize:10.5,color:"#333",lineHeight:1.7,whiteSpace:"pre-line" as const }}>{invoice.termsConditions||defaultTerms}</div></div>
        <div style={{ display:"flex",justifyContent:"flex-end",padding:"10px 14px 12px",minHeight:100 }}>
          <div style={{ textAlign:"center",minWidth:160 }}>
            {sigUrl ? <img src={sigUrl} alt="Signature" style={{ height:54,maxWidth:160,objectFit:"contain",marginBottom:4,display:"block",margin:"0 auto 4px" }}/> : invoice.showEmptySignatureBox ? <div style={{ height:48,width:140,border:"1px dashed #bbb",marginBottom:4,margin:"0 auto 4px" }}/> : <div style={{ height:48,marginBottom:4 }}/>}
            <div style={{ fontSize:10,color:"#333",marginTop:4 }}>Authorised Signatory For<br/><strong style={{ color:themeColor }}>{companyName}</strong></div>
          </div>
        </div>
      </div>
...
function LuxuryLayout({ invoice, business, template, printRef, themeColor, bgImageUrl, bgOpacity }: Omit<InvoicePaperProps,"themeLayout">) {
  const { companyName, address, gstin, bank, ifsc, bankName, accountNo, branch, logoUrl, showLogo, sigUrl, defaultTerms, grandTotal, alreadyReceived, hsnRows } = useInvoiceData(invoice, business, template);
  const fmt = fmtN; const BD = "1px solid #ccc";
  const bankPairs = [["Name",bank],["IFSC",ifsc],["Account No",accountNo],["Bank Name",bankName],["Branch",branch]].filter(([,v])=>v) as [string,string][];
  return (
    <div ref={printRef} className="ivm-invoice-paper" style={{ fontFamily:"Arial,sans-serif",fontSize:"11px",color:"#1a1a1a",background:"#fff",position:"relative",overflow:"hidden",width:"100%",maxWidth:"100%" }}>
      <BgOverlay url={bgImageUrl} opacity={bgOpacity} />
      <div style={{ position:"relative",zIndex:1 }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",padding:"14px 18px 10px",borderBottom:BD }}>
          <div style={{ display:"flex",alignItems:"flex-start",gap:10 }}>{showLogo&&logoUrl&&<img src={logoUrl} alt="Logo" style={{ height:52,width:52,objectFit:"contain",borderRadius:4,border:"1px solid #eee",flexShrink:0 }}/>}<div><div style={{ fontSize:18,fontWeight:800,color:"#111",marginBottom:2 }}>{companyName}</div>{address&&<div style={{ fontSize:10,color:"#555" }}>{address}</div>}{gstin&&<div style={{ fontSize:10,color:"#555" }}>GSTIN: {gstin}</div>}</div></div>
          <div style={{ textAlign:"right",flexShrink:0 }}><div style={{ fontSize:15,fontWeight:700,color:"#111",letterSpacing:"0.8px" }}>TAX INVOICE</div></div>
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",borderBottom:BD,fontSize:10 }}>
          {[{label:"Invoice No.",val:String(invoice.invoiceNo)},{label:"Invoice Date",val:fmtDateSlash(invoice.invoiceDate)},{label:"Due Date",val:invoice.dueDate?fmtDateSlash(invoice.dueDate):"-"}].map((f,i)=>(<div key={i} style={{ padding:"6px 12px",borderRight:i<2?BD:"none" }}><div style={{ fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase" as const,marginBottom:2 }}>{f.label}</div><div style={{ fontWeight:600,color:"#111" }}>{f.val}</div></div>))}
        </div>
        {(invoice.financedBy||invoice.salesman||invoice.challanNo||invoice.warrantyPeriod)&&(<div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",borderBottom:BD,fontSize:10 }}>{[{label:"Financed By",val:invoice.financedBy||"-"},{label:"Salesman",val:invoice.salesman||"-"},{label:"Challan No.",val:invoice.challanNo||"-"},{label:"Warranty Period",val:invoice.warrantyPeriod||"-"}].map((f,i)=>(<div key={i} style={{ padding:"6px 12px",borderRight:i<3?BD:"none" }}><div style={{ fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase" as const,marginBottom:2 }}>{f.label}</div><div style={{ fontWeight:600,color:"#111" }}>{f.val}</div></div>))}</div>)}
        <div style={{ padding:"8px 18px",borderBottom:BD,fontSize:10 }}><div style={{ fontSize:10,fontWeight:700,color:"#555",textTransform:"uppercase" as const,marginBottom:4,letterSpacing:"0.5px" }}>Bill To</div><div style={{ fontWeight:700,fontSize:12,color:"#111" }}>{invoice.party?.name||"-"}</div>{invoice.party?.billingAddress&&<div style={{ color:"#444" }}>{invoice.party.billingAddress}</div>}{invoice.party?.mobile&&<div style={{ color:"#444" }}>Mobile {invoice.party.mobile}</div>}{invoice.party?.gstin&&<div style={{ color:"#444" }}>GSTIN: {invoice.party.gstin}</div>}</div>
        <table style={{ width:"100%",borderCollapse:"collapse",fontSize:10 }}>
          <thead><tr style={{ borderBottom:BD,borderTop:BD }}>{["No","Items","Qty.","Rate","Disc.","Tax","Total"].map((h,i)=>(<th key={h} style={{ padding:"7px 10px",fontWeight:700,color:"#333",textAlign:i>=2?"right" as const:"left" as const,fontSize:10,background:"#f8f8f8" }}>{h}</th>))}</tr></thead>
          <tbody>{invoice.billItems.map((item,i)=>{const raw=item as any;const pct=Number(raw.discountPct??raw.discount_pct??0)||0;const disc=Number(raw.discountAmt??raw.discount_amt??0)||0;const taxRate=Number(raw.taxRate??raw.tax_rate??0)||0;return(<tr key={i} style={{ borderBottom:"1px solid #eee" }}><td style={{ padding:"7px 10px" }}>{i+1}</td><td style={{ padding:"7px 10px" }}>{item.name||"Item"}</td><td style={{ padding:"7px 10px",textAlign:"right" }}>{item.qty} {item.unit||"PCS"}</td><td style={{ padding:"7px 10px",textAlign:"right" }}>{fmt(Number(item.price))}</td><td style={{ padding:"7px 10px",textAlign:"right" }}>{pct>0?<span>{pct}%<br/><span style={{fontSize:9,color:"#777"}}>({fmt(Math.round(Number(item.qty)*Number(item.price)*(pct/100)*100)/100)})</span></span>:disc>0?fmt(disc):"-"}</td><td style={{ padding:"7px 10px",textAlign:"right" }}>{taxRate>0?`GST ${taxRate}%`:"-"}</td><td style={{ padding:"7px 10px",textAlign:"right",fontWeight:600 }}>{fmt(Number(item.amount))}</td></tr>);})}</tbody>
          <tfoot><tr style={{ borderTop:BD,borderBottom:BD,background:"#f8f8f8" }}><td style={{ padding:"7px 10px",fontWeight:700 }}>SUBTOTAL</td><td></td><td style={{ padding:"7px 10px",textAlign:"right",fontWeight:700 }}>{invoice.billItems.reduce((s,i)=>s+Number(i.qty),0)}</td><td></td><td style={{ padding:"7px 10px",textAlign:"right",fontWeight:700 }}>₹ {fmt(invoice.billItems.reduce((s,i)=>{const r=i as any;return s+(Number(r.discountAmt??r.discount_amt??0)||0)},0))}</td><td style={{ padding:"7px 10px",textAlign:"right",fontWeight:700 }}>₹ {fmt(hsnRows.reduce((s,r)=>s+r.cgst+r.sgst,0))}</td><td style={{ padding:"7px 10px",textAlign:"right",fontWeight:700 }}>₹ {fmt(grandTotal)}</td></tr></tfoot>
        </table>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",borderTop:BD }}>
          <div style={{ padding:"12px 18px",borderRight:BD,fontSize:10 }}><div style={{ fontWeight:700,fontSize:11,marginBottom:6 }}>Terms & Conditions</div><div style={{ color:"#555",lineHeight:1.7,marginBottom:14,whiteSpace:"pre-line" as const }}>{invoice.termsConditions||defaultTerms}</div><div style={{ fontWeight:700,fontSize:11,marginBottom:6 }}>Bank Details</div>{bankPairs.map(([k,v])=>(<div key={k} style={{ display:"flex",marginBottom:3 }}><span style={{ minWidth:80,color:"#444",flexShrink:0 }}>{k}</span><span style={{ color:"#111",fontWeight:500 }}>{v}</span></div>))}</div>
          <div style={{ padding:"12px 18px",fontSize:10 }}>
            {hsnRows.length>0&&hsnRows.map((r,i)=>(<div key={i} style={{ display:"flex",justifyContent:"space-between",marginBottom:3 }}><span style={{ color:"#555" }}>CGST @{r.rate/2}% + SGST @{r.rate/2}%</span><span>₹ {fmt(r.cgst+r.sgst)}</span></div>))}
            <div style={{ display:"flex",justifyContent:"space-between",fontWeight:700,fontSize:11,borderTop:BD,paddingTop:5,marginTop:5 }}><span>Total Amount</span><span>₹ {fmt(grandTotal)}</span></div>
            <div style={{ display:"flex",justifyContent:"space-between",marginTop:3,color:"#555" }}><span>Received Amount</span><span>₹ {fmt(alreadyReceived)}</span></div>
            <div style={{ display:"flex",justifyContent:"space-between",fontWeight:700,marginTop:3,color:safeN(grandTotal-alreadyReceived)>0?"#dc2626":"#16a34a" }}><span>Balance</span><span>₹ {fmt(safeN(grandTotal-alreadyReceived))}</span></div>
            <div style={{ marginTop:8,fontSize:10 }}><div style={{ fontWeight:700,marginBottom:2 }}>Total Amount (in words)</div><div style={{ color:"#555" }}>{numToWords(grandTotal)}</div></div>
            <div style={{ marginTop:16,minHeight:100,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"space-between",borderTop:"1px dashed #ccc",paddingTop:12 }}><div style={{ flex:1,display:"flex",alignItems:"center",justifyContent:"center",width:"100%" }}>{sigUrl&&<img src={sigUrl} alt="sig" style={{ maxHeight:70,maxWidth:160,objectFit:"contain",display:"block" }}/>}</div><div style={{ textAlign:"center",fontSize:10,marginTop:10 }}><div style={{ color:"#555",marginBottom:2 }}>Signature</div><div style={{ fontWeight:700,color:"#111" }}>{companyName}</div></div></div>
          </div>
        </div>
      </div>
    </div>
...
function StylishLayout({ invoice, business, template, printRef, themeColor, bgImageUrl, bgOpacity }: Omit<InvoicePaperProps,"themeLayout">) {
  const { companyName, address, gstin, bank, ifsc, bankName, accountNo, branch, logoUrl, showLogo, sigUrl, defaultTerms, discountAmt, roundOff, grandTotal, alreadyReceived, hsnRows } = useInvoiceData(invoice, business, template);
  const accent = themeColor === "#000000" ? "#1a1a1a" : themeColor; const fmt = fmtN;
  const bankFields = [["Name:",bank],["Bank:",bankName],["A/C No:",accountNo],["Branch:",branch],["IFSC Code:",ifsc]].filter(([,v])=>v) as [string,string][];
  return (
    <div ref={printRef} className="ivm-invoice-paper" style={{ fontFamily:"Arial,sans-serif",fontSize:"11px",background:"#fff",position:"relative",overflow:"hidden",width:"100%",maxWidth:"100%",padding:"14px 18px 16px" }}>
      <BgOverlay url={bgImageUrl} opacity={bgOpacity} />
      <div style={{ position:"relative",zIndex:1 }}>
        <div style={{ display:"flex",gap:8,marginBottom:10,fontSize:10 }}><span style={{ border:"1px solid #333",padding:"2px 8px",fontWeight:700 }}>TAX INVOICE</span><span style={{ border:"1px solid #aaa",padding:"2px 8px",color:"#555" }}>ORIGINAL FOR RECIPIENT</span></div>
        <div style={{ display:"flex",alignItems:"center",gap:14,marginBottom:10 }}>{showLogo&&logoUrl&&<img src={logoUrl} alt="Logo" style={{ height:56,width:56,objectFit:"contain",borderRadius:4,border:"1px solid #eee",flexShrink:0 }}/>}<div style={{ fontSize:22,fontWeight:700,color:accent,letterSpacing:"0.3px" }}>{companyName}</div></div>
        <div style={{ height:3,background:accent,marginBottom:0 }}/>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",background:"#f5f5f5",padding:"8px 12px",borderBottom:"1px solid #ddd",fontSize:10 }}><div><strong>Invoice No.:</strong> {invoice.invoiceNo}</div><div><strong>Invoice Date:</strong> {fmtDateSlash(invoice.invoiceDate)}</div><div><strong>Due Date:</strong> {invoice.dueDate?fmtDateSlash(invoice.dueDate):"-"}</div></div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",padding:"8px 12px",borderBottom:"1px solid #ddd",gap:16,fontSize:10 }}>
          <div><div style={{ fontWeight:700,fontSize:11,marginBottom:2 }}>BILL TO</div><div style={{ fontWeight:700,fontSize:12 }}>{invoice.party?.name||"-"}</div>{invoice.party?.billingAddress&&<div style={{ color:"#555" }}>{invoice.party.billingAddress}</div>}{invoice.party?.mobile&&<div style={{ color:"#555" }}>Mobile: {invoice.party.mobile}</div>}</div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",rowGap:3 }}>{invoice.financedBy&&<><div style={{ fontWeight:700 }}>Financed By</div><div style={{ textAlign:"right" }}>{invoice.financedBy}</div></>}{invoice.salesman&&<><div style={{ fontWeight:700 }}>Salesman</div><div style={{ textAlign:"right" }}>{invoice.salesman}</div></>}{invoice.challanNo&&<><div style={{ fontWeight:700 }}>Challan No.</div><div style={{ textAlign:"right" }}>{invoice.challanNo}</div></>}{invoice.warrantyPeriod&&<><div style={{ fontWeight:700 }}>Warranty</div><div style={{ textAlign:"right" }}>{invoice.warrantyPeriod}</div></>}</div>
        </div>
        <table style={{ width:"100%",borderCollapse:"collapse",fontSize:10 }}>
          <thead><tr style={{ borderBottom:`2px solid ${accent}` }}>{["ITEMS","QTY.","RATE","DISC.","TAX","AMOUNT"].map((h,i)=>(<th key={h} style={{ padding:"7px 8px",fontWeight:700,textAlign:i>0?"right":"left",background:"none",border:"none",fontSize:10 }}>{h}</th>))}</tr></thead>
          <tbody>{invoice.billItems.map((item,i)=>{const raw=item as any;const pct=Number(raw.discountPct??raw.discount_pct??0)||0;const disc=Number(raw.discountAmt??raw.discount_amt??0)||0;const taxRate=Number(raw.taxRate??raw.tax_rate??0)||0;return(<tr key={i} style={{ borderBottom:"1px solid #e5e5e5" }}><td style={{ padding:"6px 8px" }}>{item.name||"Item"}{item.description&&<div style={{ fontSize:9,color:"#777",marginTop:1 }}>{item.description}</div>}</td><td style={{ padding:"6px 8px",textAlign:"right" }}>{item.qty} {item.unit||"PCS"}</td><td style={{ padding:"6px 8px",textAlign:"right" }}>{fmt(Number(item.price))}</td><td style={{ padding:"6px 8px",textAlign:"right" }}>{pct>0?<span>{pct}%<br/><span style={{ fontSize:9,color:"#777" }}>({fmt(Math.round(Number(item.qty)*Number(item.price)*(pct/100)*100)/100)})</span></span>:disc>0?fmt(disc):"-"}</td><td style={{ padding:"6px 8px",textAlign:"right" }}>{taxRate>0?`GST ${taxRate}%`:"-"}</td><td style={{ padding:"6px 8px",textAlign:"right",fontWeight:600 }}>{fmt(Number(item.amount))}</td></tr>);})}</tbody>
          <tfoot><tr style={{ borderTop:`2px solid ${accent}`,borderBottom:"1px solid #ccc" }}><td style={{ padding:"6px 8px",fontWeight:700 }}>SUBTOTAL</td><td style={{ padding:"6px 8px",textAlign:"right",fontWeight:700 }}>{invoice.billItems.reduce((s,i)=>s+Number(i.qty),0)}</td><td></td><td style={{ padding:"6px 8px",textAlign:"right",fontWeight:700 }}>₹ {fmt(invoice.billItems.reduce((s,i)=>{const r=i as any;return s+(Number(r.discountAmt??r.discount_amt??0)||0)},0))}</td><td style={{ padding:"6px 8px",textAlign:"right",fontWeight:700 }}>₹ {fmt(hsnRows.reduce((s,r)=>s+r.cgst+r.sgst,0))}</td><td style={{ padding:"6px 8px",textAlign:"right",fontWeight:700 }}>₹ {fmt(grandTotal)}</td></tr></tfoot>
        </table>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",marginTop:8,gap:16,fontSize:10 }}>
          <div><div style={{ fontWeight:700,fontSize:11,marginBottom:6 }}>BANK DETAILS</div>{bankFields.map(([k,v])=>(<div key={k} style={{ display:"flex",gap:6,marginBottom:3 }}><span style={{ minWidth:72,color:"#555",flexShrink:0 }}>{k}</span><span style={{ fontWeight:500 }}>{v}</span></div>))}<div style={{ fontWeight:700,fontSize:11,marginTop:10,marginBottom:4 }}>TERMS AND CONDITIONS</div><div style={{ color:"#555",lineHeight:1.6 }}>{invoice.termsConditions||defaultTerms}</div></div>
          <div>
            {hsnRows.map((r,i)=>(<div key={i} style={{ display:"flex",justifyContent:"space-between",marginBottom:3 }}><span>CGST+SGST @{r.rate/2}%+{r.rate/2}%</span><span>₹ {fmt(r.cgst+r.sgst)}</span></div>))}
            {discountAmt>0&&<div style={{ display:"flex",justifyContent:"space-between",marginBottom:3 }}><span>Discount</span><span style={{ color:"#16a34a" }}>- ₹ {fmt(discountAmt)}</span></div>}
            {roundOff!==0&&<div style={{ display:"flex",justifyContent:"space-between",marginBottom:3 }}><span>Round Off</span><span>{roundOff>0?"+":""}₹ {fmt(Math.abs(roundOff))}</span></div>}
            <div style={{ display:"flex",justifyContent:"space-between",fontWeight:700,fontSize:12,borderTop:"1px solid #333",paddingTop:4,marginTop:4 }}><span>Total Amount</span><span>₹ {fmt(grandTotal)}</span></div>
            <div style={{ display:"flex",justifyContent:"space-between",marginTop:3,color:"#555" }}><span>Received Amount</span><span>₹ {fmt(alreadyReceived)}</span></div>
            <div style={{ textAlign:"right",marginTop:4 }}><div style={{ fontSize:10,color:"#555" }}>Total Amount (in words)</div><div style={{ fontWeight:500 }}>{numToWords(grandTotal)}</div></div>
            <div style={{ border:"1px solid #ccc",borderRadius:2,marginTop:12,minHeight:100,display:"flex",flexDirection:"column",alignItems:"flex-end",justifyContent:"space-between",padding:"10px 8px 6px" }}><div style={{ flex:1,display:"flex",alignItems:"center",justifyContent:"flex-end",width:"100%" }}>{sigUrl&&<img src={sigUrl} alt="sig" style={{ maxHeight:60,maxWidth:140,objectFit:"contain",display:"block" }}/>}</div><div style={{ textAlign:"right",fontSize:9,marginTop:8 }}><div style={{ fontWeight:700 }}>AUTHORISED SIGNATORY FOR</div><div style={{ color:accent }}>{companyName}</div></div></div>
          </div>
        </div>
      </div>
    </div>
...
function SimpleLayout({ invoice, business, template, printRef, themeColor, bgImageUrl, bgOpacity }: Omit<InvoicePaperProps,"themeLayout">) {
  const { companyName, address, gstin, bank, ifsc, bankName, accountNo, branch, logoUrl, showLogo, sigUrl, defaultTerms, discountAmt, roundOff, grandTotal, alreadyReceived, hsnRows } = useInvoiceData(invoice, business, template);
  const fmt = fmtN; const accent = themeColor && themeColor !== "#000000" ? themeColor : "#868e96";
  const bankFields = [["Name",bank],["Bank",bankName],["A/C No.",accountNo],["Branch",branch],["IFSC Code",ifsc]].filter(([,v])=>v) as [string,string][];
  return (
    <div ref={printRef} className="ivm-invoice-paper" style={{ fontFamily:"Arial,sans-serif",fontSize:"11px",background:"#fff",position:"relative",overflow:"hidden",width:"100%",maxWidth:"100%",padding:"18px 20px 20px" }}>
      <BgOverlay url={bgImageUrl} opacity={bgOpacity} />
      <div style={{ position:"relative",zIndex:1 }}>
        <div style={{ display:"flex",gap:8,marginBottom:10,fontSize:10 }}><span style={{ border:"1px solid #333",padding:"2px 8px",fontWeight:700 }}>TAX INVOICE</span><span style={{ border:"1px solid #aaa",padding:"2px 8px",color:"#555" }}>ORIGINAL FOR RECIPIENT</span></div>
        <div style={{ display:"flex",alignItems:"flex-start",gap:14,marginBottom:10,paddingBottom:10,borderBottom:`2px solid ${accent}` }}>{showLogo&&logoUrl&&<img src={logoUrl} alt="Logo" style={{ height:52,width:52,objectFit:"contain",borderRadius:4,border:"1px solid #eee",flexShrink:0 }}/>}<div style={{ flex:1 }}><div style={{ fontSize:18,fontWeight:800,color:accent }}>{companyName}</div><div style={{ fontSize:10,color:"#555",marginTop:3 }}>{address}</div><div style={{ fontSize:10,color:"#555" }}>GSTIN: {gstin}</div></div><div style={{ textAlign:"right",fontSize:10,flexShrink:0 }}><div style={{ fontWeight:700,fontSize:13,color:"#111",marginBottom:4 }}>Invoice #{invoice.invoiceNo}</div><div style={{ color:"#555" }}>Date: {fmtDateSlash(invoice.invoiceDate)}</div>{invoice.dueDate&&<div style={{ color:"#555" }}>Due: {fmtDateSlash(invoice.dueDate)}</div>}</div></div>
        {(invoice.financedBy||invoice.salesman||invoice.challanNo||invoice.warrantyPeriod)&&(<div style={{ display:"flex",flexWrap:"wrap",gap:"6px 20px",fontSize:10,padding:"6px 0",borderBottom:"1px solid #e5e5e5",marginBottom:10 }}>{invoice.financedBy&&<span><strong>Financed By:</strong> {invoice.financedBy}</span>}{invoice.salesman&&<span><strong>Salesman:</strong> {invoice.salesman}</span>}{invoice.challanNo&&<span><strong>Challan No.:</strong> {invoice.challanNo}</span>}{invoice.warrantyPeriod&&<span><strong>Warranty:</strong> {invoice.warrantyPeriod}</span>}</div>)}
        <div style={{ fontSize:10,marginBottom:12,padding:"8px 10px",background:"#f9f9f9",borderRadius:4,borderLeft:`3px solid ${accent}` }}><div style={{ fontWeight:700,color:accent,marginBottom:3,textTransform:"uppercase" as const,fontSize:9,letterSpacing:"0.5px" }}>Bill To</div><div style={{ fontWeight:700,fontSize:12,color:"#111" }}>{invoice.party?.name||"-"}</div>{invoice.party?.billingAddress&&<div style={{ color:"#555" }}>{invoice.party.billingAddress}</div>}{invoice.party?.mobile&&<div style={{ color:"#555" }}>Mobile: {invoice.party.mobile}</div>}{invoice.party?.gstin&&<div style={{ color:"#555" }}>GSTIN: {invoice.party.gstin}</div>}</div>
        <table style={{ width:"100%",borderCollapse:"collapse",fontSize:10,marginBottom:10 }}>
          <thead><tr style={{ borderBottom:`2px solid ${accent}`,borderTop:`1px solid ${accent}` }}>{["S.No","Item","Qty","Rate","Disc.","Tax","Amount"].map((h,i)=>(<th key={h} style={{ padding:"6px 8px",fontWeight:700,textAlign:i>=2?"right":"left",fontSize:10 }}>{h}</th>))}</tr></thead>
          <tbody>{invoice.billItems.map((item,idx)=>{const raw=item as any;const pct=Number(raw.discountPct??raw.discount_pct??0)||0;const disc=Number(raw.discountAmt??raw.discount_amt??0)||0;const taxRate=Number(raw.taxRate??raw.tax_rate??0)||0;return(<tr key={idx} style={{ borderBottom:"1px solid #eee" }}><td style={{ padding:"5px 8px" }}>{idx+1}</td><td style={{ padding:"5px 8px" }}><div style={{ fontWeight:600 }}>{item.name||"Item"}</div>{item.description&&<div style={{ fontSize:9,color:"#777" }}>{item.description}</div>}</td><td style={{ padding:"5px 8px",textAlign:"right" }}>{item.qty} {item.unit||"PCS"}</td><td style={{ padding:"5px 8px",textAlign:"right" }}>{fmt(Number(item.price))}</td><td style={{ padding:"5px 8px",textAlign:"right" }}>{pct>0?<span>{pct}%<br/><span style={{fontSize:9,color:"#777"}}>({fmt(Math.round(Number(item.qty)*Number(item.price)*(pct/100)*100)/100)})</span></span>:disc>0?fmt(disc):"-"}</td><td style={{ padding:"5px 8px",textAlign:"right" }}>{taxRate>0?`${taxRate}%`:"-"}</td><td style={{ padding:"5px 8px",textAlign:"right",fontWeight:600 }}>{fmt(Number(item.amount))}</td></tr>);})}</tbody>
          <tfoot><tr style={{ borderTop:`2px solid ${accent}` }}><td colSpan={6} style={{ padding:"6px 8px",textAlign:"right",fontWeight:700,fontSize:11 }}>Grand Total</td><td style={{ padding:"6px 8px",textAlign:"right",fontWeight:700,fontSize:11 }}>₹ {fmt(grandTotal)}</td></tr><tr><td colSpan={6} style={{ padding:"4px 8px",textAlign:"right",fontSize:10,color:"#555" }}>Amount Received</td><td style={{ padding:"4px 8px",textAlign:"right",fontSize:10 }}>₹ {fmt(alreadyReceived)}</td></tr><tr><td colSpan={6} style={{ padding:"4px 8px",textAlign:"right",fontSize:10,fontWeight:700 }}>Balance</td><td style={{ padding:"4px 8px",textAlign:"right",fontSize:10,fontWeight:700,color:safeN(grandTotal-alreadyReceived)>0?"#dc2626":"#16a34a" }}>₹ {fmt(safeN(grandTotal-alreadyReceived))}</td></tr></tfoot>
        </table>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,fontSize:10 }}>
          <div><div style={{ fontWeight:700,fontSize:11,marginBottom:5,color:accent }}>BANK DETAILS</div>{bankFields.map(([k,v])=>(<div key={k} style={{ display:"flex",gap:6,marginBottom:2 }}><span style={{ minWidth:72,color:"#777",flexShrink:0 }}>{k}</span><span style={{ fontWeight:500,color:"#222" }}>{v}</span></div>))}<div style={{ fontWeight:700,fontSize:11,marginTop:10,marginBottom:4,color:accent }}>TERMS AND CONDITIONS</div><div style={{ color:"#555",lineHeight:1.6,whiteSpace:"pre-line" as const }}>{invoice.termsConditions||defaultTerms}</div></div>
          <div>{hsnRows.map((r,i)=>(<div key={i} style={{ display:"flex",justifyContent:"space-between",marginBottom:2 }}><span style={{ color:"#555" }}>CGST+SGST @{r.rate/2}%+{r.rate/2}%</span><span>₹ {fmt(r.cgst+r.sgst)}</span></div>))}{discountAmt>0&&<div style={{ display:"flex",justifyContent:"space-between",marginBottom:2 }}><span style={{ color:"#555" }}>Discount</span><span style={{ color:"#16a34a" }}>- ₹ {fmt(discountAmt)}</span></div>}{roundOff!==0&&<div style={{ display:"flex",justifyContent:"space-between",marginBottom:2 }}><span style={{ color:"#555" }}>Round Off</span><span>{roundOff>0?"+":""}₹ {fmt(Math.abs(roundOff))}</span></div>}<div style={{ display:"flex",justifyContent:"space-between",fontWeight:700,fontSize:11,borderTop:`1px solid ${accent}`,paddingTop:4,marginTop:4 }}><span>Total Amount</span><span style={{ color:accent }}>₹ {fmt(grandTotal)}</span></div><div style={{ fontSize:10,marginTop:6 }}><div style={{ fontWeight:700,color:"#333" }}>Amount in words</div><div style={{ color:"#555" }}>{numToWords(grandTotal)}</div></div><div style={{ marginTop:14,minHeight:100,display:"flex",flexDirection:"column",alignItems:"flex-end",justifyContent:"space-between",borderTop:"1px dashed #ccc",paddingTop:10 }}><div style={{ flex:1,display:"flex",alignItems:"center",justifyContent:"flex-end",width:"100%" }}>{sigUrl&&<img src={sigUrl} alt="sig" style={{ maxHeight:60,maxWidth:140,objectFit:"contain" }}/>}</div><div style={{ textAlign:"right",fontSize:9,marginTop:8 }}><div style={{ fontWeight:700 }}>Authorised Signatory For</div><div style={{ color:accent }}>{companyName}</div></div></div></div>
        </div>
      </div>
    </div>
  );
...
function ModernLayout({ invoice, business, template, printRef, themeColor, bgImageUrl, bgOpacity }: Omit<InvoicePaperProps,"themeLayout">) {
  const { companyName, address, gstin, bank, ifsc, bankName, accountNo, branch, logoUrl, showLogo, sigUrl, defaultTerms, discountAmt, roundOff, grandTotal, alreadyReceived, hsnRows } = useInvoiceData(invoice, business, template);
  const fmt = fmtN; const accent = themeColor && themeColor !== "#000000" ? themeColor : "#e8590c";
  const bankFields = [["Name",bank],["Bank",bankName],["A/C No.",accountNo],["Branch",branch],["IFSC Code",ifsc]].filter(([,v])=>v) as [string,string][];
  return (
    <div ref={printRef} className="ivm-invoice-paper" style={{ fontFamily:"Arial,sans-serif",fontSize:"11px",background:"#fff",position:"relative",overflow:"hidden",width:"100%",maxWidth:"100%" }}>
      <BgOverlay url={bgImageUrl} opacity={bgOpacity} />
      <div style={{ position:"relative",zIndex:1 }}>
        <div style={{ background:accent,padding:"16px 22px",color:"#fff",display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}><div style={{ display:"flex",alignItems:"center",gap:14 }}>{showLogo&&logoUrl&&<img src={logoUrl} alt="Logo" style={{ height:56,width:56,objectFit:"contain",borderRadius:4,border:"2px solid rgba(255,255,255,0.4)",flexShrink:0,background:"rgba(255,255,255,0.15)" }}/>}<div><div style={{ fontSize:20,fontWeight:800,letterSpacing:"0.3px" }}>{companyName}</div><div style={{ fontSize:10,opacity:0.85,marginTop:3 }}>{address}</div><div style={{ fontSize:10,opacity:0.85 }}>GSTIN: {gstin}</div></div></div><div style={{ textAlign:"right",flexShrink:0 }}><div style={{ fontSize:16,fontWeight:700,letterSpacing:"1px",opacity:0.9 }}>TAX INVOICE</div><div style={{ fontSize:11,marginTop:4 }}><div><strong>Invoice No.:</strong> {invoice.invoiceNo}</div><div><strong>Date:</strong> {fmtDateSlash(invoice.invoiceDate)}</div>{invoice.dueDate&&<div><strong>Due Date:</strong> {fmtDateSlash(invoice.dueDate)}</div>}</div></div></div>
        {(invoice.financedBy||invoice.salesman||invoice.challanNo||invoice.warrantyPeriod||invoice.eWayBillNo)&&(<div style={{ background:`${accent}18`,padding:"6px 22px",fontSize:10,display:"flex",flexWrap:"wrap",gap:"6px 22px",borderBottom:`1px solid ${accent}30` }}>{invoice.financedBy&&<span><strong>Financed By:</strong> {invoice.financedBy}</span>}{invoice.salesman&&<span><strong>Salesman:</strong> {invoice.salesman}</span>}{invoice.challanNo&&<span><strong>Challan No.:</strong> {invoice.challanNo}</span>}{invoice.warrantyPeriod&&<span><strong>Warranty:</strong> {invoice.warrantyPeriod}</span>}{invoice.eWayBillNo&&<span><strong>E-Way Bill:</strong> {invoice.eWayBillNo}</span>}</div>)}
        <div style={{ padding:"10px 22px",borderBottom:`1px solid #eee`,display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,fontSize:10 }}><div><div style={{ fontWeight:700,color:accent,fontSize:9,textTransform:"uppercase" as const,letterSpacing:"0.5px",marginBottom:4 }}>Bill To</div><div style={{ fontWeight:700,fontSize:12,color:"#111" }}>{invoice.party?.name||"-"}</div>{invoice.party?.billingAddress&&<div style={{ color:"#555" }}>{invoice.party.billingAddress}</div>}{invoice.party?.mobile&&<div style={{ color:"#555" }}>Mobile: {invoice.party.mobile}</div>}{invoice.party?.gstin&&<div style={{ color:"#555" }}>GSTIN: {invoice.party.gstin}</div>}</div>{invoice.shipTo&&(<div><div style={{ fontWeight:700,color:accent,fontSize:9,textTransform:"uppercase" as const,letterSpacing:"0.5px",marginBottom:4 }}>Ship To</div><div style={{ fontWeight:700,fontSize:12,color:"#111" }}>{invoice.shipTo.name}</div>{invoice.shipTo.billingAddress&&<div style={{ color:"#555" }}>{invoice.shipTo.billingAddress}</div>}{invoice.shipTo.mobile&&<div style={{ color:"#555" }}>Mobile: {invoice.shipTo.mobile}</div>}</div>)}</div>
        <table style={{ width:"100%",borderCollapse:"collapse",fontSize:10 }}>
          <thead><tr style={{ background:accent,color:"#fff" }}>{["S.No","Item","HSN","Qty","Rate","Disc.","Tax","Amount"].map((h,i)=>(<th key={h} style={{ padding:"7px 8px",fontWeight:700,textAlign:i>=3?"right":"left",fontSize:10,color:"#fff" }}>{h}</th>))}</tr></thead>
          <tbody>{invoice.billItems.map((item,idx)=>{const raw=item as any;const pct=Number(raw.discountPct??raw.discount_pct??0)||0;const disc=Number(raw.discountAmt??raw.discount_amt??0)||0;const taxRate=Number(raw.taxRate??raw.tax_rate??0)||0;return(<tr key={idx} style={{ background:idx%2===0?"#fff":`${accent}08`,borderBottom:"1px solid #eee" }}><td style={{ padding:"6px 8px" }}>{idx+1}</td><td style={{ padding:"6px 8px" }}><div style={{ fontWeight:600 }}>{item.name||"Item"}</div>{item.description&&<div style={{ fontSize:9,color:"#777" }}>{item.description}</div>}</td><td style={{ padding:"6px 8px" }}>{item.hsn||"-"}</td><td style={{ padding:"6px 8px",textAlign:"right" }}>{item.qty} {item.unit||"PCS"}</td><td style={{ padding:"6px 8px",textAlign:"right" }}>{fmt(Number(item.price))}</td><td style={{ padding:"6px 8px",textAlign:"right" }}>{pct>0?<span>{pct}%<br/><span style={{fontSize:9,color:"#777"}}>({fmt(Math.round(Number(item.qty)*Number(item.price)*(pct/100)*100)/100)})</span></span>:disc>0?fmt(disc):"-"}</td><td style={{ padding:"6px 8px",textAlign:"right" }}>{taxRate>0?`${taxRate}%`:"-"}</td><td style={{ padding:"6px 8px",textAlign:"right",fontWeight:600 }}>{fmt(Number(item.amount))}</td></tr>);})}</tbody>
          <tfoot><tr style={{ background:`${accent}18`,borderTop:`2px solid ${accent}` }}><td colSpan={7} style={{ padding:"6px 8px",textAlign:"right",fontWeight:700 }}>Grand Total</td><td style={{ padding:"6px 8px",textAlign:"right",fontWeight:700,color:accent }}>₹ {fmt(grandTotal)}</td></tr><tr><td colSpan={7} style={{ padding:"4px 8px",textAlign:"right",fontSize:10,color:"#555" }}>Amount Received</td><td style={{ padding:"4px 8px",textAlign:"right",fontSize:10 }}>₹ {fmt(alreadyReceived)}</td></tr><tr><td colSpan={7} style={{ padding:"4px 8px",textAlign:"right",fontSize:10,fontWeight:700 }}>Balance Due</td><td style={{ padding:"4px 8px",textAlign:"right",fontSize:10,fontWeight:700,color:safeN(grandTotal-alreadyReceived)>0?"#dc2626":"#16a34a" }}>₹ {fmt(safeN(grandTotal-alreadyReceived))}</td></tr></tfoot>
        </table>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,padding:"12px 22px",fontSize:10,borderTop:`2px solid ${accent}` }}>
          <div><div style={{ fontWeight:700,fontSize:11,color:accent,marginBottom:5 }}>BANK DETAILS</div>{bankFields.map(([k,v])=>(<div key={k} style={{ display:"flex",gap:6,marginBottom:3 }}><span style={{ minWidth:72,color:"#777",flexShrink:0 }}>{k}</span><span style={{ fontWeight:500,color:"#222" }}>{v}</span></div>))}<div style={{ fontWeight:700,fontSize:11,color:accent,marginTop:10,marginBottom:4 }}>TERMS AND CONDITIONS</div><div style={{ color:"#555",lineHeight:1.6,whiteSpace:"pre-line" as const }}>{invoice.termsConditions||defaultTerms}</div></div>
          <div>{hsnRows.map((r,i)=>(<div key={i} style={{ display:"flex",justifyContent:"space-between",marginBottom:3 }}><span style={{ color:"#555" }}>CGST+SGST @{r.rate/2}%+{r.rate/2}%</span><span>₹ {fmt(r.cgst+r.sgst)}</span></div>))}{discountAmt>0&&<div style={{ display:"flex",justifyContent:"space-between",marginBottom:3 }}><span style={{ color:"#555" }}>Discount</span><span style={{ color:"#16a34a" }}>- ₹ {fmt(discountAmt)}</span></div>}{roundOff!==0&&<div style={{ display:"flex",justifyContent:"space-between",marginBottom:3 }}><span style={{ color:"#555" }}>Round Off</span><span>{roundOff>0?"+":""}₹ {fmt(Math.abs(roundOff))}</span></div>}<div style={{ display:"flex",justifyContent:"space-between",fontWeight:700,fontSize:12,borderTop:`1px solid ${accent}`,paddingTop:4,marginTop:4 }}><span>Total Amount</span><span style={{ color:accent }}>₹ {fmt(grandTotal)}</span></div><div style={{ fontSize:10,marginTop:6 }}><div style={{ fontWeight:700,color:"#333" }}>Amount in words</div><div style={{ color:"#555" }}>{numToWords(grandTotal)}</div></div><div style={{ marginTop:14,minHeight:100,display:"flex",flexDirection:"column",alignItems:"flex-end",justifyContent:"space-between",borderTop:"1px dashed #ccc",paddingTop:10 }}><div style={{ flex:1,display:"flex",alignItems:"center",justifyContent:"flex-end",width:"100%" }}>{sigUrl&&<img src={sigUrl} alt="sig" style={{ maxHeight:60,maxWidth:140,objectFit:"contain" }}/>}</div><div style={{ textAlign:"right",fontSize:9,marginTop:8 }}><div style={{ fontWeight:700 }}>Authorised Signature for</div><div style={{ color:accent }}>{companyName}</div></div></div></div>
        </div>
      </div>
    </div>
  );
...
function BillbookA5Layout({ invoice, business, template, printRef, themeColor, bgImageUrl, bgOpacity }: Omit<InvoicePaperProps,"themeLayout">) {
  const { companyName, address, gstin, bank, ifsc, bankName, accountNo, branch, logoUrl, showLogo, sigUrl, defaultTerms, grandTotal, alreadyReceived } = useInvoiceData(invoice, business, template);
  const fmt = fmtN; const BD = "1px solid #ccc"; const accent = themeColor && themeColor !== "#000000" ? themeColor : "#2c5282";
  const bankPairs = [["Name",bank],["IFSC Code",ifsc],["Account No",accountNo],["Bank",bankName||branch]].filter(([,v])=>v) as [string,string][];
  return (
    <div ref={printRef} className="ivm-invoice-paper" style={{ fontFamily:"Arial,sans-serif",fontSize:"11px",color:"#1a1a1a",background:"#fff",position:"relative",overflow:"hidden",width:"100%",maxWidth:"100%" }}>
      <BgOverlay url={bgImageUrl} opacity={bgOpacity} />
      <div style={{ position:"relative",zIndex:1 }}>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",borderBottom:BD }}><div style={{ padding:"10px 14px",borderRight:BD,display:"flex",alignItems:"flex-start",gap:10 }}>{showLogo&&logoUrl&&<img src={logoUrl} alt="Logo" style={{ height:44,width:44,objectFit:"contain",borderRadius:4,border:"1px solid #eee",flexShrink:0 }}/>}<div><div style={{ fontSize:14,fontWeight:800,color:accent,marginBottom:2 }}>{companyName}</div>{address&&<div style={{ fontSize:9.5,color:"#555" }}>{address}</div>}{gstin&&<div style={{ fontSize:9.5,color:"#555" }}>GSTIN: {gstin}</div>}</div></div><div style={{ padding:"10px 14px" }}><div style={{ fontSize:9,fontWeight:700,color:"#888",textTransform:"uppercase" as const,marginBottom:4,letterSpacing:"0.5px" }}>Bill To</div><div style={{ fontSize:13,fontWeight:800,color:"#111",marginBottom:2 }}>{invoice.party?.name||"-"}</div>{invoice.party?.billingAddress&&<div style={{ fontSize:9.5,color:"#555" }}>{invoice.party.billingAddress}</div>}{invoice.party?.mobile&&<div style={{ fontSize:9.5,color:"#555" }}>Mobile: {invoice.party.mobile}</div>}{invoice.party?.gstin&&<div style={{ fontSize:9.5,color:"#555" }}>GSTIN: {invoice.party.gstin}</div>}</div></div>
        <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",borderBottom:BD,fontSize:10 }}>{[{label:"Invoice No.",val:String(invoice.invoiceNo)},{label:"Invoice Date",val:fmtDateSlash(invoice.invoiceDate)},{label:"Due Date",val:invoice.dueDate?fmtDateSlash(invoice.dueDate):"-"},{label:"Financed By",val:invoice.financedBy||"-"}].map((f,i)=>(<div key={i} style={{ padding:"5px 10px",borderRight:i<3?BD:"none" }}><div style={{ fontSize:8.5,fontWeight:700,color:"#888",textTransform:"uppercase" as const,marginBottom:1 }}>{f.label}</div><div style={{ fontWeight:600,color:"#111",fontSize:10 }}>{f.val}</div></div>))}</div>
        {(invoice.salesman||invoice.challanNo||invoice.warrantyPeriod||invoice.eWayBillNo)&&(<div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",borderBottom:BD,fontSize:10 }}>{[{label:"Salesman",val:invoice.salesman||"-"},{label:"Challan No.",val:invoice.challanNo||"-"},{label:"Warranty Period",val:invoice.warrantyPeriod||"-"},{label:"E-Way Bill",val:invoice.eWayBillNo||"-"}].map((f,i)=>(<div key={i} style={{ padding:"5px 10px",borderRight:i<3?BD:"none" }}><div style={{ fontSize:8.5,fontWeight:700,color:"#888",textTransform:"uppercase" as const,marginBottom:1 }}>{f.label}</div><div style={{ fontWeight:600,color:"#111",fontSize:10 }}>{f.val}</div></div>))}</div>)}
        <table style={{ width:"100%",borderCollapse:"collapse",fontSize:10 }}>
          <thead><tr style={{ borderBottom:BD,background:"#f7f7f7" }}>{["S.NO.","ITEMS","QTY.","RATE","DISC.","AMOUNT"].map((h,i)=>(<th key={h} style={{ padding:"6px 10px",fontWeight:700,color:"#333",textAlign:i>=2?"right" as const:"left" as const,fontSize:10 }}>{h}</th>))}</tr></thead>
          <tbody>{invoice.billItems.map((item,i)=>{const raw=item as any;const pct=Number(raw.discountPct??raw.discount_pct??0)||0;const disc=Number(raw.discountAmt??raw.discount_amt??0)||0;return(<tr key={i} style={{ borderBottom:"1px solid #eee" }}><td style={{ padding:"6px 10px" }}>{i+1}</td><td style={{ padding:"6px 10px" }}>{item.name||"Item"}</td><td style={{ padding:"6px 10px",textAlign:"right" }}>{item.qty} {item.unit||"PCS"}</td><td style={{ padding:"6px 10px",textAlign:"right" }}>{fmt(Number(item.price))}</td><td style={{ padding:"6px 10px",textAlign:"right" }}>{pct>0?<span>{pct}%<br/><span style={{fontSize:9,color:"#777"}}>({fmt(Math.round(Number(item.qty)*Number(item.price)*(pct/100)*100)/100)})</span></span>:disc>0?fmt(disc):"-"}</td><td style={{ padding:"6px 10px",textAlign:"right",fontWeight:600 }}>{fmt(Number(item.amount))}</td></tr>);})}</tbody>
          <tfoot><tr style={{ borderTop:BD,background:"#f7f7f7" }}><td style={{ padding:"6px 10px",fontWeight:700 }}>TOTAL</td><td></td><td style={{ padding:"6px 10px",textAlign:"right",fontWeight:700 }}>{invoice.billItems.reduce((s,i)=>s+Number(i.qty),0)}</td><td></td><td style={{ padding:"6px 10px",textAlign:"right",fontWeight:700 }}>₹ {fmt(invoice.billItems.reduce((s,i)=>{const r=i as any;return s+(Number(r.discountAmt??r.discount_amt??0)||0)},0))}</td><td style={{ padding:"6px 10px",textAlign:"right",fontWeight:700 }}>₹ {fmt(grandTotal)}</td></tr><tr><td colSpan={5} style={{ padding:"5px 10px",textAlign:"right",fontSize:10,color:"#444" }}>Received Amount:</td><td style={{ padding:"5px 10px",textAlign:"right",fontSize:10 }}>₹ {fmt(alreadyReceived)}</td></tr><tr><td colSpan={5} style={{ padding:"5px 10px",textAlign:"right",fontWeight:700,fontSize:10 }}>Balance:</td><td style={{ padding:"5px 10px",textAlign:"right",fontWeight:700,fontSize:10,color:safeN(grandTotal-alreadyReceived)>0?"#dc2626":"#16a34a" }}>₹ {fmt(safeN(grandTotal-alreadyReceived))}</td></tr></tfoot>
        </table>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",borderTop:BD }}><div style={{ padding:"10px 14px",borderRight:BD,fontSize:10 }}><div style={{ fontWeight:700,marginBottom:5 }}>Bank Details</div>{bankPairs.map(([k,v])=>(<div key={k} style={{ display:"flex",marginBottom:2 }}><span style={{ minWidth:76,color:"#555",flexShrink:0 }}>{k}:</span><span style={{ fontWeight:500,color:"#111" }}>{v}</span></div>))}<div style={{ fontWeight:700,marginTop:10,marginBottom:5 }}>Terms and Conditions</div><div style={{ color:"#555",lineHeight:1.6,whiteSpace:"pre-line" as const }}>{invoice.termsConditions||defaultTerms}</div></div><div style={{ padding:"10px 14px",fontSize:10,display:"flex",flexDirection:"column",justifyContent:"space-between" }}><div><div style={{ fontSize:10,color:"#555",marginBottom:4 }}>{numToWords(grandTotal)}</div></div><div style={{ minHeight:80,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-end",gap:4,marginTop:12,borderTop:"1px dashed #ccc",paddingTop:10 }}>{sigUrl&&<img src={sigUrl} alt="sig" style={{ maxHeight:50,maxWidth:130,objectFit:"contain",display:"block",marginBottom:4 }}/>}<div style={{ fontSize:10,fontWeight:700,color:"#111" }}>Authorised Signatory For</div><div style={{ fontSize:10,color:accent }}>{companyName}</div></div></div></div>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 14px",borderTop:BD,background:"#f7f7f7",fontSize:10 }}><span style={{ fontWeight:700,letterSpacing:"0.5px" }}>BILL OF SUPPLY</span><span style={{ color:"#555",border:"1px solid #aaa",padding:"1px 8px" }}>ORIGINAL FOR RECIPIENT</span></div>
      </div>
    </div>
  );
}
...
function InvoicePaper({ invoice, business, template, printRef, themeLayout, themeColor, bgImageUrl, bgOpacity }: InvoicePaperProps) {
  if (themeLayout === "luxury") return <LuxuryLayout invoice={invoice} business={business} template={template} printRef={printRef} themeColor={themeColor} bgImageUrl={bgImageUrl} bgOpacity={bgOpacity} />;
  if (themeLayout === "stylish") return <StylishLayout invoice={invoice} business={business} template={template} printRef={printRef} themeColor={themeColor} bgImageUrl={bgImageUrl} bgOpacity={bgOpacity} />;
  if (themeLayout === "simple") return <SimpleLayout invoice={invoice} business={business} template={template} printRef={printRef} themeColor={themeColor} bgImageUrl={bgImageUrl} bgOpacity={bgOpacity} />;
  if (themeLayout === "modern") return <ModernLayout invoice={invoice} business={business} template={template} printRef={printRef} themeColor={themeColor} bgImageUrl={bgImageUrl} bgOpacity={bgOpacity} />;
  if (themeLayout === "billbook-a5") return <BillbookA5Layout invoice={invoice} business={business} template={template} printRef={printRef} themeColor={themeColor} bgImageUrl={bgImageUrl} bgOpacity={bgOpacity} />;
  return <AdvancedGSTLayout invoice={invoice} business={business} template={template} printRef={printRef} themeColor={themeColor} bgImageUrl={bgImageUrl} bgOpacity={bgOpacity} />;
}
    ...
    <>
      <div className="ivm-overlay" onClick={onClose}>
        <div className="ivm-shell" onClick={e => e.stopPropagation()}>
          ...
          <div className="ivm-actionbar">
            <div className="ivm-actionbar-left">
              <div className="ivm-action-group"><button className="ivm-action-btn" onClick={handlePrint}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 6 2 18 2 18 9"/><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"/><rect x="6" y="14" width="12" height="8"/></svg>Print PDF</button><button className="ivm-action-split"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 12 15 18 9"/></svg></button></div>
              <div className="ivm-action-group"><button className="ivm-action-btn" onClick={handleDownload}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>Download PDF</button><button className="ivm-action-split"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 12 15 18 9"/></svg></button></div>
              <div className="ivm-action-group"><button className="ivm-action-btn"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>Share</button><button className="ivm-action-split"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="6 9 12 15 18 9"/></svg></button></div>
            </div>
            <div className="ivm-actionbar-right">
              <button className="ivm-record-btn" onClick={e => { e.stopPropagation(); setShowRecordPayment(true); }}>
          ...
          <div className="ivm-body">
            <div className="ivm-preview-area">
              ...
              <InvoicePaper invoice={invoice} business={business} template={template} printRef={printRef} themeLayout={themeLayout} themeColor={themeColor} bgImageUrl={bgImageUrl} bgOpacity={bgOpacity} />
            </div>
            <div className="ivm-sidebar">
              <div className="ivm-sidebar-title">Payment History</div>
              <div className="ivm-ph-row"><span>Invoice Amount</span><strong>₹{grandTotal.toLocaleString("en-IN")}</strong></div>
              <div className="ivm-ph-row"><span>Total Amount Received</span><strong>₹{alreadyReceived.toLocaleString("en-IN")}</strong></div>
              {alreadyReceived > 0 && (<div className="ivm-ph-entry"><div className="ivm-ph-entry-top"><span>Payment Received</span><strong style={{ color:"#16a34a" }}>₹{alreadyReceived.toLocaleString("en-IN")}</strong></div><div className="ivm-ph-entry-date">{fmtDate(invoice.createdAt)}</div></div>)}
              <div style={{ flex:1 }}/>
              <div className="ivm-ph-total-row"><span>Total Amount Received</span><strong>₹{alreadyReceived.toLocaleString("en-IN")}</strong></div>
