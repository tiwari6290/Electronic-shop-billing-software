      ...
      <div className="aim-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="aim-toolbar">
          <div className="aim-search-wrap">
            ...
            />
            <button className="aim-barcode-btn" title="Scan barcode">
              ...
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{ textAlign: "center", padding: "60px 20px", color: "#9ca3af", fontSize: 14 }}>
                    Scan items to add them to your invoice
                  </td>
                </tr>
              ) : (
                filtered.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className={`aim-tr ${isSelected ? "aim-tr--selected" : ""}`}>
                      ...
                      <td className="aim-td aim-td--right">₹{item.salesPrice.toLocaleString("en-IN")}</td>
                      <td className="aim-td aim-td--right">
                        {item.purchasePrice > 0 ? `₹${item.purchasePrice.toLocaleString("en-IN")}` : "–"}
                      </td>
