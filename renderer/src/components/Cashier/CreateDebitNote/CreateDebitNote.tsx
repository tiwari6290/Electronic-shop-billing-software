...
type DNPage   = "list"|"create";
interface DNItem   { id:number; name:string; hsn:string; qty:number; price:number; discount:number; tax:number; }
interface DNCharge { id:number; label:string; amount:number; taxRate:string; }
interface DebitNote {
...
interface DNSettings { prefixEnabled:boolean; prefix:string; sequenceNumber:number; showItemImage:boolean; }
...
function CreateDebitNotePage({
  ...
  // FIX 2: noteNo is always derived from seqNo prop (= dnSettings.sequenceNumber from root)
  const [noteNo]=useState(String(seqNo));
  const [noteDate]=useState(todayStr());
  ...
  const [roundOffDir,setRoundOffDir]=useState("+Add");
  ...
  const openAddItems=()=>{
    const init:Record<number,number>={};items.forEach(i=>{init[i.id]=i.qty;});
  ...
  const updItem=(id:number,f:keyof DNItem,v:string)=>setItems(p=>p.map(i=>i.id===id?{...i,[f]:isNaN(Number(v))?v:Number(v)}:i));
  const addCharge=()=>setCharges(p=>[...p,{id:Date.now(),label:"",amount:0,taxRate:"No Tax Applicable"}]);
  const updCharge=(id:number,f:keyof DNCharge,v:string|number)=>setCharges(p=>p.map(c=>c.id===id?{...c,[f]:v}:c));
  ...
  // FIX 3: passes built note to parent which saves + remounts form
  const handleSaveAndNew=()=>{
    if(!selectedParty){showT("Please select a party first");return;}
    onSavedAndNew(buildNote());
  };

  return (
    <div className="cpi-page">
      {/* FIX 1: back arrow is its own button; title is a separate <span> — never truncates */}
      <div className="cpi-topbar">
        ...
        <div className="cpi-topbar-right">
          ...
          {/* FIX 3: calls handleSaveAndNew instead of just a toast */}
                ...
                <tr key={item.id}>
                  ...
                  <td>
                    <div style={{display:"flex",alignItems:"center",gap:3}}>
                      ...
                      <input className="qty-input" style={{flex:1}} type="number" value={item.price} onChange={e=>updItem(item.id,"price",e.target.value)}/>
                    </div>
                  </td>
                  <td>
                    <div style={{display:"flex",alignItems:"center",gap:3}}>
                      <span style={{fontSize:11,color:"#98a2b3"}}>₹</span>
                      <input className="qty-input" style={{flex:1}} type="number" value={item.discount} onChange={e=>updItem(item.id,"discount",e.target.value)}/>
                    </div>
                  </td>
                  <td>
          ...
          <div className="summary-panel">
            ...
            {!showDiscount
              ...
              :<div className="discount-row">
                  ...
                  <div className="discount-controls">
                    ...
                    <input type="number" className="discount-input" value={discountVal===0?"":discountVal} onChange={e=>setDiscountVal(Number(e.target.value))} placeholder="0"/>
            ...
            <div className="tcs-row">
              <label className="tcs-label">
                <input type="checkbox" checked={applyTCS} onChange={e=>setApplyTCS(e.target.checked)} style={{marginRight:6}}/>
                Apply TCS
              </label>
            </div>
            <div className="round-off-row">
              <label className="round-label">
                <input type="checkbox" checked={roundOff} onChange={e=>setRoundOff(e.target.checked)} style={{marginRight:6}}/>
                Auto Round Off
              </label>
              {roundOff&&(
                <div style={{display:"flex",alignItems:"center",gap:6,marginLeft:"auto"}}>
                  <select className="charge-tax-select" value={roundOffDir} onChange={e=>setRoundOffDir(e.target.value)} style={{width:80}}>
                    <option>+Add</option><option>-Reduce</option>
                  </select>
                  <span style={{fontSize:12,color:"#667085"}}>₹</span>
                  <input type="number" className="charge-amt-input" value={roundOffVal===0?"":roundOffVal} onChange={e=>setRoundOffVal(Number(e.target.value))} placeholder="0" style={{width:64}}/>
                </div>
              )}
            </div>
      ...
      {showSettings&&(
        <div className="pi-overlay" onClick={()=>setShowSettings(false)}>
          <div className="pi-modal dn-settings-modal" onClick={e=>e.stopPropagation()}>
            ...
            <div className="pi-s-block">
              <div className="pi-s-top">
                ...
                <label className="toggle">
                  <input type="checkbox" checked={dnSettings.prefixEnabled} onChange={e=>setDnSettings(p=>({...p,prefixEnabled:e.target.checked}))}/>
              ...
              {dnSettings.prefixEnabled&&(
                <>
                  <div className="pi-s-fields">
                    <div><label>Prefix</label><input placeholder="Prefix" value={dnSettings.prefix} onChange={e=>setDnSettings(p=>({...p,prefix:e.target.value}))}/></div>
                    {/* FIX 2: editing this updates root dnSettings.sequenceNumber which feeds seqNo prop on next mount */}
                    <div><label>Sequence Number</label><input type="number" value={dnSettings.sequenceNumber} onChange={e=>setDnSettings(p=>({...p,sequenceNumber:Number(e.target.value)}))}/></div>
                  </div>
            ...
            <div className="pi-s-block">
              <div className="pi-s-top">
                ...
                <label className="toggle">
                  <input type="checkbox" checked={dnSettings.showItemImage} onChange={e=>setDnSettings(p=>({...p,showItemImage:e.target.checked}))}/>
                  <span className="toggle-slider"/>
                </label>
              </div>
            </div>
...
══════════════════════════════════════════════════════════ */
function DebitNoteListPage({notes,setNotes,onCreateNew,dnSettings,setDnSettings}:{
  notes:DebitNote[];
  ...
  return (
    <div className="dn-list-page">
        ...
        <div ref={searchRef} style={{position:"relative"}}>
          ...
          {showSearch&&(
            <div className="pr-search-popup">
              ...
              <input className="pi-search-input" placeholder="Search by party name or debit note number" value={query} onChange={e=>setQuery(e.target.value)} autoFocus style={{width:260}}/>
            </div>
          )}
        </div>
            ...
            {displayed.length===0
              ...
              :displayed.map(n=>(
                <tr key={n.id}>
                  ...
                  <td><button className="tdot-btn" onClick={e=>handleDot(e,n.id)}><IC.Dots/></button></td>
                </tr>
              ))
            }
          </tbody>
      ...
      {/* Settings Modal — same as on the create page */}
      {showSettings&&(
        <div className="pi-overlay" onClick={()=>setShowSettings(false)}>
          <div className="pi-modal dn-settings-modal" onClick={e=>e.stopPropagation()}>
            <div className="pi-modal-head">
...
export default function DebitNoteModule() {
  ...
  // FIX 2: dnSettings.sequenceNumber is the SINGLE source of truth for the next note number.
  // The old code had a separate seqNo state that drifted from dnSettings inside the form.
  const [dnSettings,setDnSettings]=useState<DNSettings>({
    prefixEnabled:true, prefix:"", sequenceNumber:2, showItemImage:true
  });

  // FIX 3: incrementing formKey forces React to fully unmount+remount CreateDebitNotePage
  const [formKey,setFormKey]=useState(0);

  const handleSaved=(n:DebitNote)=>{
    setNotes(p=>[n,...p]);
    // bump so the next note gets the right number
    setDnSettings(s=>({...s,sequenceNumber:s.sequenceNumber+1}));
    setPage("list");
  };

  // FIX 3: save to list, bump seq, stay on create page with a completely fresh form
  const handleSavedAndNew=(n:DebitNote)=>{
    setNotes(p=>[n,...p]);
    setDnSettings(s=>({...s,sequenceNumber:s.sequenceNumber+1}));
    setFormKey(k=>k+1); // new key → React destroys old form and mounts a fresh blank one
  };

  if(page==="create"){
    return <CreateDebitNotePage
      key={`create-${formKey}`}         // FIX 2+3: remounts on every Save&New
      seqNo={dnSettings.sequenceNumber}  // FIX 2: always reads from settings
      dnSettings={dnSettings}
      setDnSettings={setDnSettings}
      onBack={()=>setPage("list")}
      onSaved={handleSaved}
      onSavedAndNew={handleSavedAndNew}  // FIX 3
      allParties={allParties}
      setAllParties={setAllParties}
    />;
  }
  return <DebitNoteListPage notes={notes} setNotes={setNotes} onCreateNew={()=>setPage("create")} dnSettings={dnSettings} setDnSettings={setDnSettings}/>;
}